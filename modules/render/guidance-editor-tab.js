...
function geIconPickerHTML(fieldId, currentIcon, label) {
  ...
  return '<div class="ge-field"><label>' + label + '</label>'
    ...
    + '<label class="ge-icon-upload-btn" title="Upload PNG/APNG">\uD83D\uDCC2 Upload<input type="file" accept="image/*" class="d-none" onchange="geUploadIcon(\\'' + fieldId + '\\',this)"></label>'
  ...
  body.innerHTML = _geCfg.worlds.map((world, wi) => {
    ...
    return \`<div class="ge-world" data-wi="\${wi}">
      <div class="ge-world-row \${isWorldOpen && _geSelected.catIdx == null ? 'active' : ''}"
        ...
        <span class="text-16">\${geRenderIcon(world.icon, '🌍', 16)}</span>
        <span class="flex-1">\${world.label || world.id}</span>
...
<div style="display:flex;gap:8px;margin-top:8px">
  ...
  const tiersHTML = (card.tiers || []).map((t, ti) => {
    ...
    const tierIconRow = '<div class="ge-tier-extra"><div class="ge-icon-picker" style="gap:6px;align-items:center">'
      ...
      + '<label class="ge-icon-upload-btn" style="padding:4px 10px;font-size:11px" title="Upload tier icon">📂 Upload<input type="file" accept="image/*" class="d-none" onchange="geUploadTierIcon(\\'' + tierIconId + '\\',' + wi + ',' + ci + ',' + ki + ',' + ti + ',this)"></label>'
      + '</div></div>';
...
<div class="ge-form-section" style="margin-bottom:16px;padding-bottom:10px">
  ...
  \${closeCards.map(function(card) {
    ...
    return \`<div class="ge-preview-card-row close-to-next">
      ...
      <span class="flex-1">\${card.worldLabel} › \${card.label}</span>
...
<div class="ge-preview">\${worlds.map(w => \`
  <div class="ge-preview-world">
    <div class="ge-preview-world-hdr" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'':'none'">
      <span class="text-16">\${geRenderIcon(w.icon,'\ud83c\udf0d',16)}</span>
...
function geCustomExtractorsHTML() {
  ...
  return '<div class="ge-form-section">'
    ...
    + '<button class="ge-btn secondary" class="mt-2" onclick="geToggleCustomExtForm(true)" id="ge_cx_new_btn">+ New Custom Extractor</button>'
    + '</div>'
    + '<div class="ge-cx-form" id="ge_cx_form" class="d-none">'
  ...
  try {
    ...
    if (!res.ok || !data.versions) {
      body.innerHTML = '<div class="ge-empty"><div class="ge-empty-icon">📂</div><p>No history available yet.</p>'
        + '<button class="ge-btn secondary" class="mt-2" onclick="geCreateBackup()">💾 Create Manual Backup</button></div>';
      return;
    }
    ...
    body.innerHTML = '<div class="ge-form-section">'
      ...
      + '<div class="mb-3"><button class="ge-btn secondary" onclick="geCreateBackup()">💾 Create Manual Backup</button></div>'
      ...
      + '</div>'
      + '<div class="mt-2"><button class="ge-btn secondary" onclick="geRenderEditor()">← Back to Editor</button></div>';
  } catch(e) {
