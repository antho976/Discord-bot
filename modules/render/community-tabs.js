...
import { renderGuidanceEditorTab } from './guidance-editor-tab.js';
...
function _inlineFieldRow(label, inputHTML) {
  return `<div><label class="sub-sm" style="margin-bottom:3px">${label}</label>${inputHTML}</div>`;
}

function _inlineInput(id, placeholder, type = 'text', extra = '') {
  return `<input id="${id}" type="${type}" placeholder="${placeholder}" class="input-field" ${extra}>`;
}
...
function _inlineSelect(id, options) {
  return `<select id="${id}" class="input-field">${options.map(o => `<option value="${o.value}">${o.label}</option>`).join('')}</select>`;
}
...
function ifSave_${id}(){
  ...
  fetch('/api/features/${api}',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
    .then(function(r){return r.json()}).then(function(d){
      ...
      if(d.success){if(st)st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){if(st)st.innerHTML='';},3000);}
      else{if(st)st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}
...
function ifSave_birthdays(){
  ...
  fetch('/api/features/birthdays',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_birthdays_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
`;
}
...
export function renderTab(tab, userTier, subTab){
 ...
 if(tab==='overview') {
  ...
  // Sparkline SVG
  const _sparkSvg = _sparkData.length >= 2 ? '<svg width="120" height="30" viewBox="0 0 120 30"><polyline fill="none" stroke="#9146ff" stroke-width="1.5" points="' + _sparkPoints + '"/></svg>' : '<span class="clr-muted text-11">Not enough data</span>';

  // YouTube health line
  const _ytHealthLine = yaStatus.health?.lastError ? '<span class="clr-red">⚠️ YouTube error: ' + yaStatus.health.lastError + '</span>' : '<span class="clr-green">✅ YouTube checker healthy</span>';
...
<div data-ov-section="community" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:8px;margin-bottom:10px">
  <div class="card" style="padding:10px 14px;margin:0">
    <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span class="text-13">👥</span><span style="font-size:11px;font-weight:700;color:#e0e0e0">Community</span></div>
    <div style="display:flex;gap:12px;align-items:center;font-size:11px">
      <div><span class="clr-muted">VIPs</span> <span style="color:#9146ff;font-weight:700" id="ovVipCount">—</span></div>
      <div><span class="clr-muted">Giveaways</span> <span style="color:#4caf50;font-weight:700">${_activeGiveaways}</span></div>
      <div><span class="clr-muted">Servers</span> <span style="color:#5865f2;font-weight:700">${client.guilds.cache.size}</span></div>
    </div>
  </div>
  <div class="card" style="padding:10px 14px;margin:0">
    <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span class="text-13">🎯</span><span style="font-size:11px;font-weight:700;color:#e0e0e0">Monthly Goals</span></div>
...
<div style="margin-bottom:10px">
  <div data-ov-section="admin" class="card" style="padding:10px 14px;margin:0">
    <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span class="text-13">📊</span><span style="font-size:11px;font-weight:700;color:#e0e0e0">API Rate Limits</span></div>
    <div id="ovRateLimitsWrap" class="text-11">
      <span class="clr-muted">Loading…</span>
    </div>
  </div>
</div>
  ...
  <div class="ov-body">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;align-items:start">
      ...
      <div style="background:linear-gradient(135deg,#1a3a1a,#1a2a1a);border:1px solid #4caf5033;border-radius:8px;padding:16px;text-align:center;margin-bottom:10px">
        ...
        <div class="text-12 clr-muted">Uptime: <strong style="color:#e0e0e0">${_uptimeStr}</strong></div>
      </div>` : _schedNoStream ? `
    ...
    <div style="margin-top:12px;padding:10px 12px;background:#2a2f3a;border-radius:6px;display:flex;align-items:center;gap:10px">
      ...
      <div class="flex-1">
        ...
        <div class="text-11 clr-muted">${_goalStreams} / ${_goalStreamTarget} streams · ${_daysLeftInMonth} day${_daysLeftInMonth !== 1 ? 's' : ''} remaining</div>
      </div>
...
<div data-ov-section="admin" class="card" style="border-left:3px solid #4caf50">
  ...
  <div id="serverHealthCard" style="padding-top:8px;border-top:1px solid #2a2f3a">
    <div class="clr-muted text-12">Loading health score...</div>
  </div>
</div>
<script>
(function(){
  fetch('/api/features/server-health').then(function(r){return r.json()}).then(function(d){
    ...
    if(d.history&&d.history.length>1){
      ...
      historyHtml='<div class="mt-3" style="margin-top:10px"><div class="sub-sm" style="font-size:10px;margin-bottom:4px">Recent trend ('+pts.length+' checks)</div><svg viewBox="0 0 200 44" style="width:100%;height:40px"><polyline fill="none" stroke="'+color+'" stroke-width="1.5" points="'+sPoints+'"/></svg></div>';
    }
    var html='<div style="display:flex;align-items:center;gap:16px"><div style="width:72px;height:72px;border-radius:50%;border:4px solid '+color+';display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:700;color:'+color+';flex-shrink:0">'+score+'</div><div class="flex-1"><div style="font-size:14px;color:#e0e0e0;font-weight:600">Server Health: '+label+'</div><div class="text-11 clr-muted mt-1">Based on activity, engagement, retention, and moderation.</div><div style="margin-top:6px;height:6px;background:#1a1d28;border-radius:3px;overflow:hidden"><div style="width:'+score+'%;height:100%;background:'+color+';border-radius:3px;transition:width 0.5s"></div></div>'+historyHtml+'</div></div>';
...
<div data-ov-section="admin" class="card" style="border-left:3px solid #4caf50">
  ...
  <div style="padding-top:8px;border-top:1px solid #2a2f3a">
    <div class="d-grid cols-2 gap-4">
      <div><label class="sub-sm" style="margin-bottom:3px">API URL (HTTPS)</label><input id="if_apiPoll_url" placeholder="https://api.example.com/data" class="input-field"></div>
      <div><label class="sub-sm" style="margin-bottom:3px">JSON Path</label><input id="if_apiPoll_path" placeholder="data.value" class="input-field"></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:8px">
      <div><label class="sub-sm" style="margin-bottom:3px">Channel ID</label><input id="if_apiPoll_ch" placeholder="Channel ID" class="input-field"></div>
      <div><label class="sub-sm" style="margin-bottom:3px">Interval (min, 5-1440)</label><input id="if_apiPoll_interval" type="number" min="5" max="1440" value="30" class="input-field"></div>
      <div><label class="sub-sm" style="margin-bottom:3px">Label</label><input id="if_apiPoll_label" placeholder="My API" class="input-field"></div>
    </div>
...
function saveApiPoll(){
  ...
  fetch('/api/features/api-polling',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_apiPoll_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
...
<div data-ov-section="status" class="card ov-collapsible" data-collapsed="true">
  ...
  <div class="ov-body" class="d-none">
...
function showVIPs() {
  ...
  fetch('/api/vips')
    ...
    .then(function(data) {
      ...
      if (data.error) {
        content.innerHTML = '<p class="clr-red">Error: ' + data.error + '</p><p style="color:#999;font-size:13px">Your Twitch token may need channel:read:vips scope</p>';
    ...
    .catch(function(err) {
      document.getElementById('vipContent').innerHTML = '<p class="clr-red">Failed: ' + err.message + '</p>';
    });
}
...
function ovRefreshThumb() {
  fetch('/api/stream-thumbnail').then(function(r){return r.json()}).then(function(d){
    ...
    var loading = document.getElementById('ovThumbLoading');
    if (loading) loading.innerHTML = '<span class="clr-red">Failed to load preview</span>';
  });
}
ovRefreshThumb();
...
function ovLoadRateLimits() {
  fetch('/api/rate-limits').then(function(r){return r.json()}).then(function(d){
    ...
    // Twitch
    html += '<div style="background:#2a2f3a;padding:8px 10px;border-radius:6px"><div class="sub-sm" style="font-size:10px;margin-bottom:4px">🟣 Twitch API</div>';
    html += '<div class="d-flex gap-3 items-center flex-wrap">';
    ...
    html += '<div style="background:#2a2f3a;padding:8px 10px;border-radius:6px"><div class="sub-sm" style="font-size:10px;margin-bottom:4px">🔵 Discord</div>';
    html += '<div class="d-flex gap-3 items-center flex-wrap">';
    html += '<span class="clr-muted">WS:</span> <span style="color:' + dColor + ';font-weight:700">' + (dPing >= 0 ? dPing + 'ms' : '—') + '</span>';
    html += '<span class="clr-muted">Guilds:</span> <span style="color:#5865f2;font-weight:700">' + (d.discord ? d.discord.guilds : 0) + '</span>';
    html += '</div></div>';
    // YouTube
    html += '<div style="background:#2a2f3a;padding:8px 10px;border-radius:6px"><div class="sub-sm" style="font-size:10px;margin-bottom:4px">🔴 YouTube Feeds</div>';
    html += '<div class="d-flex gap-3 items-center flex-wrap">';
    html += '<span class="clr-muted">Polls:</span> <span style="color:#ff4444;font-weight:700">' + (d.youtube ? d.youtube.calls : 0) + '</span>';
    html += '<span class="clr-muted">/min:</span> <span style="color:#ff4444;font-weight:700">' + (d.youtube ? d.youtube.callsMinute : 0) + '</span>';
    html += '</div></div>';
    // Overall
    html += '<div style="background:#2a2f3a;padding:8px 10px;border-radius:6px"><div class="sub-sm" style="font-size:10px;margin-bottom:4px">📈 Total</div>';
    html += '<div class="d-flex gap-3 items-center flex-wrap">';
    html += '<span class="clr-muted">Calls/min:</span> <span style="color:#5865f2;font-weight:700">' + d.callsThisMinute + '</span>';
    html += '<span class="clr-muted">Total:</span> <span style="color:#e0e0e0;font-weight:700">' + d.totalCalls + '</span>';
    html += '</div></div>';
    html += '</div>';
    wrap.innerHTML = html;
  }).catch(function(){
...
function _schedBuildOverlay(overlay) {
  ...
  for (var d = 1; d <= daysInMonth; d++) {
    ...
    calCells += '<div class="schedDayCell" data-date="' + dateStr + '" style="background:' + bg + ';border:2px solid ' + bc + ';border-radius:8px;padding:6px 4px;text-align:center;cursor:pointer;transition:all .12s;user-select:none;min-height:58px">' +
      ...
      '<input type="checkbox" class="schedDayCb" data-date="' + dateStr + '" ' + (active ? 'checked' : '') + ' class="d-none">' +
...
<div class="card">
  ...
  <div style="display:grid;grid-template-columns:1fr auto auto auto;gap:8px;align-items:center;margin-bottom:8px">
    <input id="log-search" placeholder="🔍 Search message, type, or source..." class="m-0">
    <select id="log-sort" style="width:auto" class="m-0">
      <option value="newest">Newest first</option>
      <option value="oldest">Oldest first</option>
    </select>
    <select id="log-source" style="width:auto" class="m-0">
    ...
    <button class="small" id="log-reset" style="width:auto" class="m-0">Reset</button>
  </div>

  <!-- Controls Row 2 -->
  <div style="display:grid;grid-template-columns:auto auto auto 1fr;gap:8px;align-items:center;margin-bottom:10px">
    <select id="log-time-range" style="width:auto" class="m-0">
  ...
  <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;align-items:center">
    ...
    <button class="small" data-type="info" style="width:auto" class="m-0">ℹ️ Info</button>
    <button class="small" data-type="live" style="width:auto" class="m-0">🔴 Live</button>
    <button class="small" data-type="offline" style="width:auto" class="m-0">⚫ Offline</button>
    <button class="small" data-type="error" style="width:auto" class="m-0">❌ Error</button>
    <button class="small" data-type="warn" style="width:auto" class="m-0">⚠️ Warn</button>
    <button class="small" data-type="milestone" style="width:auto" class="m-0">🏆 Milestone</button>
    <span class="flex-1"></span>
    <button class="small" id="log-export" style="width:auto" class="m-0">⬇️ Export</button>
    <button class="small danger" id="log-clear" style="width:auto" class="m-0">🗑️ Clear</button>
  </div>
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;padding:8px 12px;background:#1a1d28;border:1px solid #2a2f3a;border-radius:6px">
    ...
    <div style="display:flex;align-items:center;gap:12px">
      ...
      <span id="log-results" class="text-12 clr-muted">0 results</span>
    </div>
  </div>
  ...
  <div id="logbox-container" style="max-height:calc(100vh - 380px);min-height:500px;overflow-y:auto;border:1px solid #3a3a42;border-radius:8px;background:#17171b">
    ...
    <div id="log-load-more" style="text-align:center;padding:12px;display:none">
      ...
      <div class="text-11 clr-muted mt-1"><span id="log-loaded-count">0</span> / <span id="log-total-filtered">0</span> loaded</div>
    </div>
...
initSSE();
  ...
  if (tab === 'stats-revenue') {
    ...
    if ((TIER_LEVELS[userTier] || 0) < TIER_LEVELS.admin) return '<div class="card"><h2>🔒 Access Denied</h2><p class="clr-muted">Revenue predictions require admin or higher access.</p></div>';
  ...
  if (tab === 'idleon-guidance') return renderGuidanceEditorTab(userTier);
  ...
  if (tab === 'smartbot-templates') return renderSmartBotTemplatesTab(smartBot);
...
export function renderModerationTab() {
  ...
  if (auditEntries.length === 0) {
    ...
    for (const [date, items] of Object.entries(byDate)) {
      ...
      items.forEach(e => {
        ...
        auditHtml += '<div style="padding:5px 8px;margin-bottom:3px;background:#1e1f22;border-radius:4px;border-left:3px solid ' + actionColor + ';font-size:12px"><span class="clr-muted text-11">' + time + '</span> <span style="color:#9146ff;font-weight:600">' + (e.user||'Unknown') + '</span> <span style="color:' + actionColor + ';font-weight:500">' + (e.action||'action') + '</span>' + (e.details ? ' <span class="clr-muted">\u2014 ' + String(e.details).slice(0,80) + '</span>' : '') + '</div>';
      });
    }
  }
  ...
  if (cases.length === 0) {
    ...
    casesHtml = '<div style="max-height:400px;overflow-y:auto"><table style="width:100%;font-size:12px"><thead style="position:sticky;top:0;background:#1a1a2e;z-index:1"><tr><th class="p-2 text-left">#</th><th class="p-2 text-left">Type</th><th class="p-2 text-left">User</th><th class="p-2 text-left">Moderator</th><th class="p-2 text-left">Reason</th><th class="p-2 text-left">Date</th><th class="p-2 text-left">💬</th></tr></thead><tbody>';
    cases.forEach((c, idx) => {
  ...
  if (warnings.length === 0) {
    ...
    warnsHtml = '<div style="max-height:300px;overflow-y:auto"><table style="width:100%;font-size:12px"><thead style="position:sticky;top:0;background:#1a1a2e;z-index:1"><tr><th class="p-2 text-left">User</th><th class="p-2 text-left">Warned By</th><th class="p-2 text-left">Reason</th><th class="p-2 text-left">Date</th></tr></thead><tbody>';
    warnings.forEach(w => {
...
<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:8px;margin-bottom:14px">
  <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#9146ff">${totalCases}</div><div class="text-10 clr-muted">Cases</div></div>
  <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#f39c12">${totalWarnings}</div><div class="text-10 clr-muted">Warnings</div></div>
  <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#e74c3c">${bans}</div><div class="text-10 clr-muted">Bans</div></div>
  <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#e67e22">${kicks}</div><div class="text-10 clr-muted">Kicks</div></div>
  <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#f39c12">${timeouts}</div><div class="text-10 clr-muted">Timeouts</div></div>
  <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#2ecc71">${uniqueMods.length}</div><div class="text-10 clr-muted">Moderators</div></div>
</div>
    ...
    <div>
      <div class="mb-4">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
          ...
          <span class="text-11 clr-muted">Click 💬 to open discussion</span>
        </div>
        ${casesHtml}
      </div>
    </div>
...
<div id="modSubAudit" style="display:block;margin-top:14px">
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px">
    <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#9146ff">${auditEntries.length}</div><div class="text-10 clr-muted">Total Actions</div></div>
    <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#2ecc71">${auditUsers.length}</div><div class="text-10 clr-muted">Active Accounts</div></div>
    <div class="panel" style="padding:10px;background:#2b2d31"><div style="font-size:20px;font-weight:700;color:#f39c12">${Object.keys(byDate).length}</div><div class="text-10 clr-muted">Active Days</div></div>
  </div>
  ...
  if (filtered.length === 0) {
    ...
    for (var date in byDate) {
      ...
      byDate[date].forEach(function(e) {
        ...
        html += '<div style="display:flex;align-items:center;justify-content:space-between;padding:5px 8px;margin-bottom:3px;background:#1e1f22;border-radius:4px;border-left:3px solid ' + actionColor + ';font-size:12px"><div><span class="clr-muted text-11">' + time + '</span> <span style="color:#9146ff;font-weight:600">' + (e.user||'Unknown') + '</span> <span style="color:' + actionColor + ';font-weight:500">' + (e.action||'action') + '</span>' + hasSnapshot + (e.details ? ' <span class="clr-muted">\\u2014 ' + String(e.details).slice(0,80) + '</span>' : '') + '</div><button onclick="revertAuditEntry(\\''+String(e.ts).replace(/'/g,"\\\\'")+'\\',\\''+(e.action||'').replace(/'/g,"\\\\'")+'\\',\\''+(e.user||'').replace(/'/g,"\\\\'")+'\\',\\''+(String(e.details||'').slice(0,60)).replace(/'/g,"\\\\'")+'\\'" style="padding:2px 8px;background:#e74c3c22;color:#e74c3c;border:1px solid #e74c3c44;border-radius:4px;cursor:pointer;font-size:10px;width:auto;white-space:nowrap" title="' + (e.snapshot ? 'Auto-revert available' : 'Request manual revert') + '">↩ Revert</button></div>';
      });
    }
  }
...
function openCaseDiscussion(caseId, caseType, userName) {
  ...
  document.getElementById('caseDiscussionHeader').innerHTML = '<span style="font-weight:700;font-size:15px">Case: ' + caseType + '</span> <span class="clr-muted">\u2014 ' + userName + '</span> <span style="font-size:11px;color:#8b8fa3;margin-left:8px">ID: ' + caseId + '</span>';
  ...
  fetch('/api/moderation/case/comments?caseId=' + encodeURIComponent(caseId))
    ...
    .then(data => {
      ...
      container.innerHTML = data.comments.map(c => {
        ...
        return '<div style="padding:8px 10px;margin-bottom:6px;background:#2b2d31;border-radius:6px"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span style="color:#9146ff;font-weight:600;font-size:12px">' + (c.user||'Unknown') + '</span><span class="clr-muted text-10">' + time + '</span></div><div class="text-13 text-primary">' + (c.text||'') + '</div></div>';
      }).join('');
...
export function renderTicketsTab() {
  ...
  if (allItems.length === 0) {
    ...
    allItems.forEach((item, i) => {
      ...
      // Notes if any
      if (item.notes) {
        itemsHtml += '<div style="padding:6px 8px;background:#26262c;border-radius:4px;margin-bottom:6px;border-left:3px solid #5b5bff;font-size:11px"><span class="clr-muted">Notes: </span><span style="color:#b0b0b0">' + String(item.notes).slice(0,150) + '</span></div>';
      }
      // Actions row
      itemsHtml += '<div class="d-flex gap-3 items-center flex-wrap">';
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
  ...
  <div class="text-10 clr-muted">🤖 AI auto-classifies: <span style="color:#ed4245">Critical</span> · <span style="color:#e67e22">High</span> · <span style="color:#faa61a">Medium</span> · <span style="color:#3ba55c">Low</span></div>
</div>

<!-- Stats Row -->
<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:6px;margin-bottom:10px">
  <div style="padding:8px;background:#2b2d31;border-radius:6px;text-align:center"><div style="font-size:18px;font-weight:700;color:#9146ff">${totalTickets}</div><div class="text-10 clr-muted">Tickets</div></div>
  <div style="padding:8px;background:#2b2d31;border-radius:6px;text-align:center"><div style="font-size:18px;font-weight:700;color:#2ecc71">${openTickets}</div><div class="text-10 clr-muted">Open</div></div>
  <div style="padding:8px;background:#2b2d31;border-radius:6px;text-align:center"><div style="font-size:18px;font-weight:700;color:#5b5bff">${totalSuggestions}</div><div class="text-10 clr-muted">Suggestions</div></div>
  <div style="padding:8px;background:#2b2d31;border-radius:6px;text-align:center"><div style="font-size:18px;font-weight:700;color:#faa61a">${pendingSuggestions}</div><div class="text-10 clr-muted">Pending</div></div>
  <div style="padding:8px;background:#2b2d31;border-radius:6px;text-align:center"><div style="font-size:18px;font-weight:700;color:#ed4245">${criticalCount}</div><div class="text-10 clr-muted">Critical</div></div>
  <div style="padding:8px;background:#2b2d31;border-radius:6px;text-align:center"><div style="font-size:18px;font-weight:700;color:#e67e22">${highCount}</div><div class="text-10 clr-muted">High</div></div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr auto;gap:6px;margin-bottom:8px">
  ...
  <div style="background:#1e1f22;border-radius:6px;padding:8px 10px">
    ...
    <div style="display:flex;gap:4px;align-items:center">
      ...
      <span class="text-10 clr-muted">min</span>
...
<div class="card" style="padding:10px;margin-top:8px">
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
    ...
    <span class="text-10 clr-muted">— warn & auto-close</span>
...
function ifSave_ticket_idle(){
  ...
  fetch('/api/features/ticket-idle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_ticket_idle_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
...
export function renderReactionRolesTab() {
  ...
  panels.forEach((p, i) => {
    ...
    html += `<div style="padding:10px;background:#2b2d31;border-radius:6px;margin-bottom:8px;border-left:3px solid #9146ff">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
        <div style="flex:1;min-width:0"><div class="font-semibold">${p.title||'Panel '+(i+1)}</div><div class="text-12 clr-muted">Channel: ${chName ? '#'+chName.name : (p.channelId||'N/A')} | Roles: ${(p.roles||[]).length}${rolesInfo ? ' — '+rolesInfo : ''}</div></div>
  ...
  return `<div class="card"><h2>🎭 Reaction Roles</h2><p class="clr-muted">Create reaction-based role assignment panels.</p>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px">
    <div><label class="text-11 clr-muted uppercase">Title</label><input id="rrTitle" placeholder="Role Menu" class="my-1"></div>
    <div><label class="text-11 clr-muted uppercase">Channel</label>
      <div class="pos-rel">
        <input id="rrChannelSearch" placeholder="Type to search channels..." autocomplete="off" class="my-1" onfocus="rrShowDrop('rrChannelDrop')" oninput="rrFilterDrop('rrChannelDrop','rrChannelSearch','ch')">
  ...
  <div class="mt-3" style="margin-top:10px">
    <label class="text-11 clr-muted uppercase">Roles & Emojis</label>
  ...
  <button class="small" onclick="createReactionRole()" class="mt-3" style="margin-top:10px">➕ Create Panel</button></div><div class="card"><h3>📋 Active Panels (${panels.length})</h3>${html}</div>
<script>
...
export function renderScheduledMsgsTab() {
  ...
  msgs.forEach(m => {
    ...
    const status = m.sent ? '<span class="clr-green2">✅ Sent</span>' : '<span style="color:#f39c12">⏳ Pending</span>';
  ...
  return `<div class="card"><h2>📅 Scheduled Messages</h2><p class="clr-muted">Schedule messages to be sent at a specific time.</p>
  <div style="display:grid;grid-template-columns:2fr 1fr 1fr;gap:10px;margin-top:10px"><div><label class="text-11 clr-muted uppercase">Message</label><input id="schedContent" placeholder="Your message..." class="my-1"></div><div><label class="text-11 clr-muted uppercase">Channel ID</label><input id="schedChannel" placeholder="Channel ID" class="my-1"></div><div><label class="text-11 clr-muted uppercase">Send At</label><input id="schedTime" type="datetime-local" class="my-1"></div></div><button class="small" onclick="createScheduledMsg()" class="mt-2">📅 Schedule</button></div><div class="card"><h3>📋 Messages (${msgs.length})</h3>${html}</div>
<script>
...
<div class="card" style="padding:10px;margin-bottom:8px">
  <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span class="text-13">🚧</span><span style="font-size:12px;font-weight:700;color:#e0e0e0">Exclusions</span><span class="text-10 clr-muted">— bypass all rules</span></div>
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px">
    <div>
      <label class="sub-xs">Exempt Channels</label>
      <div class="pos-rel">
    ...
    <div>
      <label class="sub-xs">Exempt Users</label>
      <div class="pos-rel">
    ...
    <div>
      <label class="sub-xs">Exempt Categories</label>
      <div class="pos-rel">
...
function ifSave_auto_purge(){
  ...
  fetch('/api/features/auto-purge',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_auto_purge_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
  ...
  <div class="card" style="padding:12px;margin:0">
    <div style="display:flex;align-items:center;gap:6px;margin-bottom:10px"><span class="text-14">🛡️</span><span style="font-size:13px;font-weight:700;color:#e0e0e0">Spam & Flood</span></div>
    <div style="display:grid;gap:6px;font-size:12px">
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amSpam" ${data.antiSpam?'checked':''}> <strong>Anti-Spam</strong></label>
        <div style="display:flex;gap:6px;margin-top:4px;align-items:center;flex-wrap:wrap">
          <span class="text-10 clr-muted">Max</span><input id="amSpamThreshold" type="number" min="2" max="20" value="${data.spamThreshold||5}" style="width:42px;margin:0;padding:2px 4px">
          <span class="text-10 clr-muted">msgs /</span><input id="amSpamWindow" type="number" min="3" max="30" value="${data.spamWindowSec||5}" style="width:42px;margin:0;padding:2px 4px"><span class="text-10 clr-muted">s →</span>
      ...
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amMassMention" ${data.blockMassMentions?'checked':''}> <strong>Mass Mentions</strong></label>
        <div style="display:flex;gap:6px;margin-top:4px;align-items:center"><span class="text-10 clr-muted">Max</span><input id="amMentionLimit" type="number" min="2" max="50" value="${data.mentionLimit||5}" style="width:50px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">per msg →</span>
      ...
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amEveryoneMention" ${data.blockEveryoneMention?'checked':''}> <strong>Block @everyone/@here</strong></label>
        <div style="font-size:10px;color:#8b8fa3;margin-top:2px">Block unauthorized @everyone/@here attempts</div>
      </div>
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amDuplicates" ${data.blockDuplicates?'checked':''}> <strong>Block Duplicates</strong></label>
        <div style="display:flex;gap:6px;margin-top:4px;align-items:center">
          <span class="text-10 clr-muted">Window</span><input id="amDuplicateWindow" type="number" min="5" max="120" value="${data.duplicateWindowSec||30}" style="width:45px;margin:0;padding:2px 4px"><span class="text-10 clr-muted">s →</span>
      ...
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amSlowmode" ${data.autoSlowmode?'checked':''}> <strong>Auto Slowmode</strong></label>
        <div style="display:flex;gap:4px;margin-top:4px;align-items:center;flex-wrap:wrap">
          <input id="amSlowmodeThreshold" type="number" min="5" max="50" value="${data.slowmodeThreshold||15}" style="width:42px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">msgs/10s →</span>
          <input id="amSlowmodeDuration" type="number" min="1" max="120" value="${data.slowmodeDuration||5}" style="width:42px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">s slow, expire</span>
          <input id="amSlowmodeCooldown" type="number" min="10" max="600" value="${data.slowmodeCooldownSec||60}" style="width:45px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">s</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <!-- Right: Security -->
  <div class="card" style="padding:12px;margin:0">
    <div style="display:flex;align-items:center;gap:6px;margin-bottom:10px"><span class="text-14">🚨</span><span style="font-size:13px;font-weight:700;color:#e0e0e0">Security</span></div>
    <div style="display:grid;gap:6px;font-size:12px">
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amRaidProtection" ${data.raidProtection?'checked':''}> <strong>Raid Protection</strong></label>
        <div style="font-size:10px;color:#8b8fa3;margin-top:2px">Auto-lock on mass joins</div>
        <div style="display:flex;gap:4px;margin-top:4px;align-items:center;flex-wrap:wrap">
          <input id="amRaidJoins" type="number" min="3" max="30" value="${data.raidJoinThreshold||10}" style="width:42px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">joins in</span>
          <input id="amRaidWindow" type="number" min="5" max="120" value="${data.raidWindowSec||30}" style="width:42px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">s, expire</span>
          <input id="amRaidExpiry" type="number" min="60" max="1800" value="${data.raidExpirySec||300}" style="width:50px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">s</span>
        </div>
      </div>
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amNewAccounts" ${data.blockNewAccounts?'checked':''}> <strong>New Account Filter</strong></label>
        <div style="font-size:10px;color:#8b8fa3;margin-top:2px">Flag accounts younger than:</div>
        <div style="display:flex;gap:4px;margin-top:4px;align-items:center">
          <input id="amNewAccountAge" type="number" min="1" max="30" value="${data.newAccountAgeDays||7}" style="width:50px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">days</span>
      ...
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amAntiPhishing" ${data.antiPhishing?'checked':''}> <strong>Anti-Phishing</strong></label>
        <div style="font-size:10px;color:#8b8fa3;margin-top:2px">Block known phishing/scam links</div>
      </div>
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amAntiEmojiSpam" ${data.antiEmojiSpam?'checked':''}> <strong>Emoji Spam</strong></label>
        <div style="display:flex;gap:4px;margin-top:4px;align-items:center">
          <span class="text-10 clr-muted">Max</span><input id="amEmojiLimit" type="number" min="3" max="50" value="${data.emojiLimit||15}" style="width:50px;margin:0;padding:2px 4px"> <span class="text-10 clr-muted">emojis/msg →</span>
      ...
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amZalgo" ${data.blockZalgo?'checked':''}> <strong>Block Zalgo Text</strong></label>
        <div style="font-size:10px;color:#8b8fa3;margin-top:2px">Block glitch/corrupted text characters</div>
      </div>
      <div class="panel-dark">
        <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amAttachments" ${data.blockAttachments?'checked':''}> <strong>Block Attachments</strong></label>
        <div style="display:flex;gap:4px;margin-top:4px;align-items:center">
          <span class="text-10 clr-muted">Allowed types (empty=block all):</span>
        </div>
...
<!-- Content Filters -->
<div class="card" style="padding:10px;margin-bottom:8px">
  <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span class="text-13">🔤</span><span style="font-size:12px;font-weight:700;color:#e0e0e0">Content Filters</span></div>
  <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">
    ...
    <div style="display:flex;align-items:center;gap:4px;background:#1a1a1d;padding:4px 10px;border-radius:6px;font-size:11px">
      <span class="clr-muted">Caps ></span><input id="amCapsThreshold" type="number" min="50" max="100" value="${data.capsThreshold||70}" style="width:36px;margin:0;padding:2px"><span class="clr-muted">%</span>
      <span style="color:#8b8fa3;margin-left:4px">min</span><input id="amCapsMinLength" type="number" min="3" max="50" value="${data.capsMinLength||10}" style="width:36px;margin:0;padding:2px"><span class="clr-muted">chars</span>
    </div>
    <div style="display:flex;align-items:center;gap:4px;background:#1a1a1d;padding:4px 8px;border-radius:6px;font-size:11px">
      <span class="clr-muted">Invite →</span>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:4px">
    <div>
      <label class="sub-xs">✅ Whitelisted Domains</label>
    ...
    <div>
      <label class="sub-xs">✅ Whitelisted Patterns</label>
...
<!-- Warn Escalation -->
<div class="card" style="padding:10px;margin-bottom:8px">
  <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span class="text-13">⚡</span><span style="font-size:12px;font-weight:700;color:#e0e0e0">Warn Escalation</span><span class="text-10 clr-muted">— auto-escalate on warning count</span></div>
  ...
  <div style="display:flex;align-items:center;gap:8px;margin-top:8px;padding-top:8px;border-top:1px solid #2a2f3a">
    ...
    <span class="text-10 clr-muted">Auto-expire after</span>
    ...
    <span class="text-10 clr-muted">days</span>
  </div>
</div>
...
function saveAutomod(){
  ...
  fetch('/api/automod/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(settings)})
    .then(r=>r.json()).then(d=>{
      if(d.success){document.getElementById('amStatus').innerHTML='<span class="clr-green2">\\u2705 Saved!</span>';setTimeout(()=>{document.getElementById('amStatus').innerHTML='';},3000);}
...
<!-- Scam Protection -->
<div class="card" style="padding:10px;margin-bottom:8px">
  <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span class="text-13">🛡️</span><span style="font-size:12px;font-weight:700;color:#e0e0e0">Scam Protection</span></div>
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:11px">
    <label class="row-flex-sm" style="margin:0"><input type="checkbox" id="amScamProtection" ${data.scamProtection?'checked':''}> <strong>Enable</strong></label>
    <span class="clr-muted">Score ≥</span><input id="amScamScoreThreshold" type="number" min="1" max="20" value="${data.scamScoreThreshold||5}" style="width:40px;margin:0;padding:2px 4px;font-size:10px"><span class="clr-muted">to block</span>
    <span class="clr-muted">Action:</span>
  ...
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:11px;margin-top:6px;padding-top:6px;border-top:1px solid #2a2f3a">
    <span class="clr-muted">👥 Community Reports:</span>
    <span class="clr-muted">Pts/report</span><input id="amCommunityWeight" type="number" min="1" max="5" value="${data.communityReportWeight||2}" style="width:36px;margin:0;padding:2px 4px;font-size:10px">
    <span class="clr-muted">Min reporters</span><input id="amCommunityMinUsers" type="number" min="2" max="10" value="${data.communityReportMinUsers||2}" style="width:36px;margin:0;padding:2px 4px;font-size:10px">
...
<!-- Action Settings -->
<div class="card" style="padding:10px;margin-bottom:8px">
  <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span class="text-13">⚙️</span><span style="font-size:12px;font-weight:700;color:#e0e0e0">Action Settings</span><span class="text-10 clr-muted">— customize behavior</span></div>
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;font-size:11px;margin-bottom:6px">
    <div style="background:#1a1a1d;padding:6px 8px;border-radius:4px">
      <span class="sub-xs">🔇 Mute Duration</span>
      <div style="display:flex;align-items:center;gap:4px"><input id="amMuteDuration" type="number" min="1" max="1440" value="${data.muteDurationMin||5}" style="width:50px;margin:0;padding:2px 4px"><span class="clr-muted text-10">min</span></div>
    </div>
    <div style="background:#1a1a1d;padding:6px 8px;border-radius:4px">
      <span class="sub-xs">🗑️ Warn Auto-Delete</span>
      <div style="display:flex;align-items:center;gap:4px"><input id="amWarnDeleteSec" type="number" min="3" max="60" value="${data.warnDeleteSec||8}" style="width:50px;margin:0;padding:2px 4px"><span class="clr-muted text-10">sec</span></div>
    </div>
    <div style="background:#1a1a1d;padding:6px 8px;border-radius:4px">
      <span class="sub-xs">🔨 Ban Msg Delete</span>
      <div style="display:flex;align-items:center;gap:4px"><input id="amBanDeleteHours" type="number" min="0" max="168" value="${data.banDeleteMessageHours||24}" style="width:50px;margin:0;padding:2px 4px"><span class="clr-muted text-10">hrs</span></div>
    </div>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11px;margin-bottom:6px">
    <div style="background:#1a1a1d;padding:6px 8px;border-radius:4px">
      <span class="sub-xs">💬 Warn Message <span style="color:#666">{user} {reason}</span></span>
    ...
    <div style="background:#1a1a1d;padding:6px 8px;border-radius:4px">
      <span class="sub-xs">📩 DM Message <span style="color:#666">{server} {reason} {action}</span></span>
  ...
  <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:11px;margin:0;background:#1a1a1d;padding:6px 8px;border-radius:4px;width:fit-content"><input type="checkbox" id="amDmOnAction" ${data.dmOnAction?'checked':''}> <strong>📨 DM user on action</strong> <span class="clr-muted">— send warning via DM</span></label>
</div>
...
export function renderStarboardTab() {
  ...
  posts.forEach(p => {
    postsHtml += `<div style="padding:8px;background:#2b2d31;border-radius:6px;margin-bottom:6px;border-left:3px solid #ffd700"><div class="font-semibold">⭐ ${p.stars||0} stars</div><div class="text-12 clr-muted">${(p.content||'').slice(0,100)} — by ${p.authorName||p.authorId||'?'}</div></div>`;
  });
  return `<div class="card" style="padding:16px">
  <h2 class="m-0">⭐ Highlights & Repost</h2>
...
<div class="card"><h3 style="margin:0 0 10px">⚙️ Starboard Settings</h3><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">
  <div><label class="text-11 clr-muted uppercase">Channel</label><select id="sbChannel" class="my-1">${chOpts}</select></div>
  <div><label class="text-11 clr-muted uppercase">Min Stars</label><input id="sbMin" type="number" value="${s.minStars||3}" min="1" class="my-1"></div>
  <div><label class="text-11 clr-muted uppercase">Emoji</label><input id="sbEmoji" value="${s.emoji||'⭐'}" class="my-1"></div>
</div><button class="small" onclick="saveStarboard()" class="mt-2">💾 Save</button><div id="sbStatus" class="mt-2"></div></div>
<div class="card"><h3 style="margin:0 0 8px">🌟 Recent Starred Posts (${posts.length})</h3>${postsHtml}</div>
</div>

<!-- Admin Repost Section -->
<div id="sb-section-repost" class="d-none">
<div class="card">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">
    <div><label class="text-11 clr-muted uppercase">Enabled</label><select id="arEnabled" class="my-1"><option value="false"${!adminRepost.enabled ? ' selected' : ''}>Disabled</option><option value="true"${adminRepost.enabled ? ' selected' : ''}>Enabled</option></select></div>
    <div><label class="text-11 clr-muted uppercase">Target Channel</label>
      <div class="pos-rel">
    ...
    <div><label class="text-11 clr-muted uppercase">Emoji</label><input id="arEmoji" value="${adminRepost.emoji||'📌'}" class="my-1"></div>
  </div>
  <button class="small" onclick="saveAdminRepost()" class="mt-2">💾 Save</button><div id="arStatus" class="mt-2"></div>
</div>
</div>
...
export function renderBotStatusTab() {
  ...
  return `<div class="card"><h2>🤖 Bot Status</h2><p class="clr-muted">Real-time bot health, Discord stats, and performance metrics.</p>
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;margin-top:12px">
  ...
  <div style="padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border-radius:8px;border:1px solid #2a2f3a">
    ...
    <div class="text-11 clr-muted">RSS: ${memRss}MB (includes shared libs)</div>
  </div>
  <div style="padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border-radius:8px;border:1px solid #2a2f3a">
    <div style="font-size:11px;color:#8b8fa3;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">Discord</div>
    <div class="text-13 text-primary">Guilds: <b>${guildCount}</b></div>
    <div class="text-13 text-primary">Members: <b>${memberCount}</b></div>
    <div class="text-13 text-primary">Channels: <b>${channelCount}</b> • Roles: <b>${roleCount}</b></div>
    <div class="text-11 clr-muted">Member cache: ${cacheAge !== null ? cacheAge + 'm ago' : 'N/A'}</div>
  </div>
  <div style="padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border-radius:8px;border:1px solid #2a2f3a">
    <div style="font-size:11px;color:#8b8fa3;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px">Usage</div>
    <div class="text-13 text-primary">Commands used: <b>${totalCmds}</b></div>
    <div class="text-13 text-primary">Recent points: <b>${hourlyData.length}</b></div>
    <div class="text-13 text-primary">Cases: <b>${totalCases}</b> • Warnings: <b>${totalWarnings}</b></div>
  </div>
</div></div>`;
}
...
export function renderPetsTab(userTier) {
  ...
  return '<div class="card">'
    ...
    + '<div class="card">'
    + '<h2>🔍 Filters</h2>'
    + '<div class="mb-4"><input type="text" id="filter-search" oninput="applyFilters()" placeholder="Search pets by name..." style="width:100%;padding:8px 14px;background:#1e1f22;border:1px solid #333;border-radius:8px;color:#e0e0e0;font-size:14px;outline:none;box-sizing:border-box" onfocus="this.style.borderColor=\'#9146ff\'" onblur="this.style.borderColor=\'#333\'"></div>'
    + '<div style="display:flex;gap:12px;flex-wrap:wrap;align-items:end">'
    + '<div><label style="font-size:11px;color:#8b8fa3;text-transform:uppercase;letter-spacing:.5px">Rarity</label>'
    + '<select id="filter-rarity" onchange="applyFilters()" class="my-1"><option value="">All Rarities</option><option value="common">Common</option><option value="uncommon">Uncommon</option><option value="rare">Rare</option><option value="legendary" selected>Legendary</option></select></div>'
    + '<div><label style="font-size:11px;color:#8b8fa3;text-transform:uppercase;letter-spacing:.5px">Category</label>'
    + '<select id="filter-category" onchange="applyFilters()" class="my-1"><option value="">All Categories</option></select></div>'
    ...
    + '<select id="filter-hidden" onchange="applyFilters()" class="my-1"><option value="no">Hide Hidden</option><option value="yes">Show All</option></select></div>'
    + '<div><label style="font-size:11px;color:#8b8fa3;text-transform:uppercase;letter-spacing:.5px">Given By</label>'
    + '<select id="filter-givenby" onchange="applyFilters()" class="my-1"><option value="">All Givers</option></select></div>'
    ...
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px"><h2 id="edit-title" class="m-0">Edit Pet</h2><button onclick="closeEditModal()" style="background:none;border:none;color:#ccc;font-size:24px;cursor:pointer">&times;</button></div>'
    + '<input type="hidden" id="edit-id">'
    + '<div style="display:grid;gap:12px">'
    + '<div><label class="text-11 clr-muted uppercase">Rarity</label><select id="edit-rarity" class="my-1"><option value="common">Common</option><option value="uncommon">Uncommon</option><option value="rare">Rare</option><option value="legendary">Legendary</option></select></div>'
    + '<div><label class="text-11 clr-muted uppercase">Category</label><select id="edit-category" class="my-1"></select></div>'
    + '<div class="d-grid cols-2 gap-4"><div><label class="text-11 clr-muted uppercase">Tier Rank</label><select id="edit-tier" class="my-1"><option value="">No Tier</option><option value="S">S Rank</option><option value="A">A Rank</option><option value="B">B Rank</option><option value="C">C Rank</option><option value="D">D Rank</option></select></div>'
    + '<div><label class="text-11 clr-muted uppercase">Tier Points (0-100)</label><input type="number" id="edit-tierPoints" min="0" max="100" value="" placeholder="0-100" style="margin:4px 0;width:100%"></div></div>'
    + '<div><label class="text-11 clr-muted uppercase">Description</label><textarea id="edit-description" rows="3" style="margin:4px 0;resize:vertical"></textarea></div>'
    + '<div><label class="text-11 clr-muted uppercase">Bonus / Effect</label><input type="text" id="edit-bonus" placeholder="e.g. +10% XP, +5 Luck" class="my-1"></div>'
    + '<div><label class="text-11 clr-muted uppercase">Static Image <span class="clr-mid" style="color:#555">(.png, .jpg, .webp)</span></label>'
    ...
    + '<div id="drop-imageUrl-text" class="clr-muted text-12">📁 Drag &amp; drop image here or click to browse</div>'
    + '<input type="file" id="file-imageUrl" accept="image/png,image/jpeg,image/webp" class="d-none" onchange="handleFileSelect(this,\'imageUrl\')">'
    ...
    + '</div></div>'
    + '<div><label class="text-11 clr-muted uppercase">Animated Image <span class="clr-mid" style="color:#555">(.gif) — optional override</span></label>'
    ...
    + '<div id="drop-animatedUrl-text" class="clr-muted text-12">📁 Drag &amp; drop .gif here or click to browse</div>'
    + '<input type="file" id="file-animatedUrl" accept="image/gif" class="d-none" onchange="handleFileSelect(this,\'animatedUrl\')">'
    ...
    + '</div></div>'
    + '<div class="d-flex items-center gap-4"><input type="checkbox" id="edit-hidden"><label for="edit-hidden" style="font-size:12px;color:#b0b0b0;cursor:pointer">Hidden (hide from catalog & our pets)</label></div>'
    ...
    + '<div class="pos-rel">'
    + '<label class="text-10 clr-muted uppercase">Winner Name</label>'
    ...
    // Given By
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">'
    + '<div><label class="text-10 clr-muted uppercase">Given By</label>'
    ...
    + '<div id="giveaway-giver-other-wrap" class="d-none"><label class="text-10 clr-muted uppercase">Custom Name</label>'
    ...
    + '<div><label class="text-10 clr-muted uppercase">Notes (optional)</label>'
    ...
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h2 class="m-0">💡 Suggested Best Pet</h2><button onclick="closeSuggestModal()" style="background:none;border:none;color:#ccc;font-size:24px;cursor:pointer">&times;</button></div>'
    ...
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h2 class="m-0">🎁 Given By</h2><button onclick="closeGivenByModal()" style="background:none;border:none;color:#ccc;font-size:24px;cursor:pointer">&times;</button></div>'
    + '<p style="color:#8b8fa3;font-size:12px;margin-top:0">Who gave this pet? Pick from the list or type a new name.</p>'
    + '<div style="display:grid;gap:12px">'
    + '<div><label class="text-11 clr-muted uppercase">Select Giver</label>'
    ...
    + '<div style="background:#1e1e1e;padding:30px;border-radius:12px;max-width:400px;width:100%">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h2 id="remove-picker-title" class="m-0">Remove Pet</h2><button onclick="document.getElementById(\'remove-picker-modal\').style.display=\'none\'" style="background:none;border:none;color:#ccc;font-size:24px;cursor:pointer">&times;</button></div>'
    ...
    + '<div style="background:#1e1e1e;padding:30px;border-radius:12px;max-width:520px;width:100%;max-height:80vh;overflow-y:auto">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px"><h2 id="create-pet-title" class="m-0">➕ New Pet</h2><button onclick="closeCreatePetModal()" style="background:none;border:none;color:#ccc;font-size:24px;cursor:pointer">&times;</button></div>'
    + '<div style="display:grid;gap:12px">'
    + '<div><label class="text-11 clr-muted uppercase">📁 Category</label><div id="create-pet-category-display" style="margin:4px 0;padding:8px 12px;background:#2a2f3a;border:1px solid #3a3a42;border-radius:6px;color:#e0e0e0;font-size:13px"></div><input type="hidden" id="create-pet-category"></div>'
    + '<div style="display:grid;grid-template-columns:2fr 1fr;gap:8px"><div><label class="text-11 clr-muted uppercase">Name</label><input type="text" id="create-pet-name" placeholder="Pet name" class="my-1"></div><div><label class="text-11 clr-muted uppercase">Emoji</label><input type="text" id="create-pet-emoji" placeholder="🐾" class="my-1"></div></div>'
    + '<div class="d-grid cols-2 gap-4"><div><label class="text-11 clr-muted uppercase">Rarity</label><select id="create-pet-rarity" class="my-1"><option value="common">Common</option><option value="uncommon">Uncommon</option><option value="rare">Rare</option><option value="legendary">Legendary</option></select></div><div><label class="text-11 clr-muted uppercase">Tier</label><select id="create-pet-tier" class="my-1"><option value="">No Tier</option><option value="S">S</option><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select></div></div>'
    + '<div><label class="text-11 clr-muted uppercase">Description</label><textarea id="create-pet-desc" rows="2" style="margin:4px 0;resize:vertical" placeholder="Short description..."></textarea></div>'
    + '<div><label class="text-11 clr-muted uppercase">Bonus / Effect</label><input type="text" id="create-pet-bonus" placeholder="e.g. +10% XP" class="my-1"></div>'
    + '<div><label class="text-11 clr-muted uppercase">Static Image <span class="clr-mid" style="color:#555">(.png, .jpg, .webp)</span></label>'
    ...
    + '<div id="create-drop-imageUrl-text" class="clr-muted text-12">📁 Drag &amp; drop image here or click to browse</div>'
    + '<input type="file" id="create-file-imageUrl" accept="image/png,image/jpeg,image/webp" class="d-none" onchange="handleCreateFileSelect(this,\'imageUrl\')">'
    ...
    + '</div></div>'
    + '<div><label class="text-11 clr-muted uppercase">Animated Image <span class="clr-mid" style="color:#555">(.gif) — optional</span></label>'
    ...
    + '<div id="create-drop-animatedUrl-text" class="clr-muted text-12">📁 Drag &amp; drop .gif here or click to browse</div>'
    + '<input type="file" id="create-file-animatedUrl" accept="image/gif" class="d-none" onchange="handleCreateFileSelect(this,\'animatedUrl\')">'
    ...
    + '    \'<div style="padding:10px 18px;background:#9146ff15;border:1px solid #9146ff33;border-radius:8px;text-align:center"><div style="font-size:22px;font-weight:700;color:#9146ff">\'+owned+\'</div><div class="text-11 clr-muted">Pets Owned</div></div>\''
    + '    +\'<div style="padding:10px 18px;background:#2ecc7115;border:1px solid #2ecc7133;border-radius:8px;text-align:center"><div style="font-size:22px;font-weight:700;color:#2ecc71">\'+total+\'</div><div class="text-11 clr-muted">Available</div></div>\''
    + '    +\'<div style="padding:10px 18px;background:#3498db15;border:1px solid #3498db33;border-radius:8px;text-align:center"><div style="font-size:22px;font-weight:700;color:#3498db">\'+categories.length+\'</div><div class="text-11 clr-muted">Categories</div></div>\''
    + '    +\'<div style="padding:10px 18px;background:#f39c1215;border:1px solid #f39c1233;border-radius:8px;text-align:center"><div style="font-size:22px;font-weight:700;color:#f39c12">\'+leg+\'</div><div class="text-11 clr-muted">Legendary</div></div>\';'
    + '}'
    ...
    + '      html+=\'<button onclick="event.stopPropagation();doRemoveByGiver(\\\'\'+(g==="(no giver)"?"":g)+\'\\\',\\\'\'+ petId +\'\\\')" style="padding:10px;background:#2a2a2e;border:1px solid #444;border-radius:8px;color:#e0e0e0;cursor:pointer;text-align:left"><span style="font-weight:700">\'+ g +\'</span> <span class="clr-muted text-11">(x\'+ cnt +\')</span></button>\';'
    + '    });'
    + '    html+="</div>";'
    ...
    + '  var src=c.animatedUrl||c.imageUrl||"";'
    + '  document.getElementById("giveaway-pet-info").innerHTML=imgTag(src,c.name,c.emoji,64)+\'<div style="font-weight:700;margin-top:6px">\'+c.emoji+" "+c.name+\'</div><div class="text-11 clr-muted">\'+c.rarity+\'</div>\';'
    ...
    + '    if(!c) return;'
    + '    html+=\'<label style="display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:6px;cursor:pointer;margin-bottom:4px;background:#1e1f22"><input type="checkbox" class="random-check" value="\'+c.id+\'" checked><span>\'+c.emoji+" "+c.name+\'</span><span class="text-10 clr-muted">\'+c.rarity+\'</span></label>\';'
    + '  });'
...
export function renderPetApprovalsTab(userTier) {
  ...
  return '<div class="card">'
    ...
    + '<div style="padding:10px 18px;background:#f39c1215;border:1px solid #f39c1233;border-radius:8px;text-align:center"><div style="font-size:22px;font-weight:700;color:#f39c12" id="pending-count">' + pending.length + '</div><div class="text-11 clr-muted">Pending</div></div>'
    + '<div style="padding:10px 18px;background:#2ecc7115;border:1px solid #2ecc7133;border-radius:8px;text-align:center"><div style="font-size:22px;font-weight:700;color:#2ecc71">' + (petsData.pendingPets || []).filter(p => p.status === 'approved').length + '</div><div class="text-11 clr-muted">Approved</div></div>'
    + '<div style="padding:10px 18px;background:#e74c3c15;border:1px solid #e74c3c33;border-radius:8px;text-align:center"><div style="font-size:22px;font-weight:700;color:#e74c3c">' + (petsData.pendingPets || []).filter(p => p.status === 'rejected').length + '</div><div class="text-11 clr-muted">Rejected</div></div>'
    + '</div>'
    ...
    // Pending list
    + '<div class="card">'
    + '<h3 class="mt-0">⏳ Pending Requests</h3>'
    + '<div id="pending-list"></div>'
    + '</div>'

    // Recent history
    + '<div class="card">'
    + '<h3 class="mt-0">📋 Approval History</h3>'
    + '<div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;align-items:end">'
    + '<div style="flex:1;min-width:160px;position:relative"><label class="text-11 clr-muted uppercase">Search</label><input id="hist-search" oninput="renderHistory();histAutocomplete(this)" placeholder="Pet name, requester, approver..." style="width:100%;margin:4px 0" autocomplete="off"><div id="hist-search-suggest" style="display:none;position:absolute;top:100%;left:0;right:0;background:#1e1f22;border:1px solid #3a3a42;border-radius:0 0 6px 6px;max-height:120px;overflow-y:auto;z-index:100"></div></div>'
    + '<div><label class="text-11 clr-muted uppercase">Status</label><select id="hist-status" onchange="renderHistory()" class="my-1"><option value="">All</option><option value="approved">Approved</option><option value="rejected">Rejected</option></select></div>'
    + '<div><label class="text-11 clr-muted uppercase">Type</label><select id="hist-type" onchange="renderHistory()" class="my-1"><option value="">All</option><option value="formal">Formal</option><option value="legacy">Legacy</option></select></div>'
    + '</div>'
    ...
    + '<div style="background:#1e1e1e;padding:30px;border-radius:12px;max-width:400px;width:100%">'
    + '<h2 class="mt-0">❌ Reject Pet</h2>'
    + '<input type="hidden" id="reject-pending-id">'
    + '<div id="reject-pet-info" class="mb-4"></div>'
    + '<label class="text-11 clr-muted uppercase">Reason (optional)</label>'
    ...
    + '        +\'<div class="text-11 clr-muted mt-1">Requested by <b>\'+p.requestedByName+\'</b> \u2022 \'+timeAgo(p.requestedAt)+\'</div>\''
    ...
    + '      +\'<span class="font-semibold">\'+p.petEmoji+\' \'+p.petName+legacyBadge+\'</span>\''
    + '      +\'<span class="text-11 clr-muted">by \'+p.requestedByName+\'</span>\''
...
export function renderPetGiveawaysTab(userTier) {
  ...
  return '<div class="card">'
    ...
    // Comments Tab
    + '<div id="giveaway-comments-tab" class="d-none">'
    ...
    // Stats Tab
    + '<div id="giveaway-stats-tab" class="d-none">'
    + '<div id="giveaway-stats-content"></div>'
    + '</div>'
    
    // Bans Tab (admin only)
    + (isAdmin ? '<div id="giveaway-bans-tab" class="d-none">'
    ...
    + '  if(batch.length===0&&gvPage===0){document.getElementById("giveaway-list").innerHTML=\'<p class="clr-muted">No giveaways found.</p>\';document.getElementById("giveaway-loading").style.display="none";return;}'
...
export function renderPetStatsTab(userTier) {
  ...
  return '<div class="card">'
    ...
    + '<div class="text-11 clr-muted mt-1">Total Pets Owned</div>'
    + '</div>'
    ...
    + '<div class="text-11 clr-muted mt-1">Unique Species</div>'
    + '</div>'
    ...
    + '<div class="text-11 clr-muted mt-1">Total Available</div>'
    + '</div>'
    ...
    + '<div class="text-11 clr-muted mt-1">Collection Complete</div>'
    + '</div>'
    + '</div>'
    + '</div>'
    ...
    + '<div class="card">'
    + '<h3 class="mt-0">📦 By Category</h3>'
    + '<canvas id="categoryChart" style="max-height:300px"></canvas>'
    + '</div>'
    
    // Rarity breakdown chart
    + '<div class="card">'
    + '<h3 class="mt-0">💎 By Rarity</h3>'
    ...
    // Top 10 pets
    + '<div class="card">'
    + '<h3 class="mt-0">🏆 Top 10 Most Owned Pets</h3>'
    ...
    // Top givers
    + '<div class="card">'
    + '<h3 class="mt-0">🎁 Top 10 Pet Givers</h3>'
    + '<div style="background:#1e1f22;border:1px solid #2a2a3a;border-radius:8px;padding:12px">'
...
<div class="card" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
  <div>
    <h2 class="m-0">📊 IdleOn Guild Manager</h2>
...
<div class="card">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:8px">
    <div>
      <h2 class="m-0">⚠️ Top 5 At-Risk Members</h2>
...
(function(){
  ...
  function bonusTooltip(g){
    ...
    return'<div class="sub-sm" style="font-size:10px;margin-bottom:4px">Total: <b style="color:#b794f6">'+total+'</b></div>'+rows;
  }
  ...
  function renderGuildBreakdown(){
    ...
    if(!guilds.length){el.innerHTML='<div class="clr-muted">No guilds configured yet.</div>';return;}
  ...
  function renderTopRisk(){
    ...
    el.innerHTML=top5.map(function(m){
      ...
      return'<div class="idl-atrisk-row">'
        ...
        +'<div class="flex-1"><b>'+safe(m.name)+'</b> <span class="clr-muted text-11">'+safe(m.guild)+'</span></div>'
        +'<div class="text-12 clr-muted">'+m.days+'d away · '+fmtN(m.wgp)+' GP/wk</div>'
        +'</div>';
    }).join('');
  }
  ...
  function renderRecent(){
    ...
    el.innerHTML=events.slice(0,15).map(function(e){var d=new Date(e.t);var ts=d.toLocaleDateString()+' '+d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});return'<div style="padding:4px 0;border-bottom:1px solid #2a2f3a"><span class="clr-muted text-11">'+ts+'</span> '+e.html+'</div>'}).join('')||'<div class="clr-muted">No recent activity</div>';
  }
...
<div class="card">
  ...
  <p class="clr-muted">Full member list with inactivity tracking, streaks, kick risk, and profile cards. Click column headers to sort.</p>
</div>
...
<div class="card">
  ...
  <div id="idlCompareResult" class="mt-3" style="margin-top:10px"></div>
</div>
...
<div class="card">
  ...
  <div style="display:flex;gap:8px;align-items:center;justify-content:space-between;margin-top:10px;flex-wrap:wrap">
    <span id="idlMemInfo" class="text-12 clr-muted"></span>
    <div style="display:flex;gap:8px;align-items:center">
      <button class="small" id="idlMemPrev" class="m-0">← Prev</button>
      <span id="idlMemPage" class="text-12 clr-muted"></span>
      <button class="small" id="idlMemNext" class="m-0">Next →</button>
    </div>
  </div>
</div>
...
<div id="idlProfileModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);z-index:1000;align-items:center;justify-content:center;overflow-y:auto">
  <div style="max-width:700px;width:90%;margin:auto;background:#1e1e24;border:1px solid #3a3a42;border-radius:12px;padding:20px;max-height:90vh;overflow-y:auto">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
      <h2 id="idlProfileName" class="m-0"></h2>
      <button class="small" onclick="document.getElementById('idlProfileModal').style.display='none'" class="m-0">✕ Close</button>
    </div>
...
(function(){
  ...
  function renderRows(){
    ...
    document.getElementById('idlMemRows').innerHTML=paged.map(function(m,i){
      ...
      return'<tr class="idl-mem-row '+riskClass+'" style="cursor:pointer" data-profile="'+dn+'">'
        ...
        +'<td>'+statusPill+'</td>'
        +(canWrite?'<td class="nowrap"><button class="small" data-qa="'+dn+'" data-action="watchlist" style="margin:0;padding:2px 6px;font-size:11px" title="Toggle Watchlist">👁</button> <button class="small" data-qa="'+dn+'" data-action="note" style="margin:0;padding:2px 6px;font-size:11px" title="Add Note">📝</button></td>':'')
        +'</tr>';
    }).join('')||'<tr><td colspan="'+(canWrite?'11':'9')+'" class="text-center clr-muted">No members found</td></tr>';
  ...
  window._idlOpenProfile=function(name){
    ...
    html+='<div class="idl-kpi"><div class="label">Days Away</div><div class="val" class="text-18"><span class="idl-status-'+statusColor(d)+'">●</span> '+d+'</div></div>';
    html+='<div class="idl-kpi"><div class="label">Risk Score</div><div class="val" style="font-size:18px;color:'+(sc>=70?'#f44336':sc>=40?'#ff9800':sc>=20?'#ffc107':'#4caf50')+'">'+sc+'<span class="text-12 clr-muted">/100</span></div></div>';
    html+='<div class="idl-kpi"><div class="label">Streak</div><div class="val" class="text-18">'+st+'<span class="text-12 clr-muted">wk</span></div></div>';
    html+='<div class="idl-kpi"><div class="label">Best Streak</div><div class="val" class="text-18">'+bs+'<span class="text-12 clr-muted">wk</span></div></div>';
    html+='<div class="idl-kpi"><div class="label">Risk: Inactivity</div><div class="val" class="text-14">'+bd.inact+'<span class="text-11 clr-muted">/40</span></div></div>';
    html+='<div class="idl-kpi"><div class="label">Risk: Trend</div><div class="val" class="text-14">'+bd.trend+'<span class="text-11 clr-muted">/25</span></div></div>';
    html+='</div>';
    ...
    html+='<div style="margin-bottom:8px"><b>📝 Notes</b> <span class="clr-muted text-12">('+notes.length+')</span></div>';
    if(notes.length){html+='<div style="max-height:120px;overflow-y:auto;margin-bottom:12px;background:#17171b;border:1px solid #3a3a42;border-radius:8px;padding:8px">'+notes.map(function(n){return'<div style="padding:4px 0;border-bottom:1px solid #2a2f3a;font-size:12px"><span class="clr-muted">'+new Date(n.date).toLocaleDateString()+' — '+safe(n.author||'?')+'</span><br>'+safe(n.text)+'</div>'}).join('')+'</div>';}
    ...
    if(tl.length){
      ...
      tl.slice().sort(function(a,b){return(b.date||0)-(a.date||0)}).slice(0,20).forEach(function(e){
        html+='<div style="padding:3px 0;border-bottom:1px solid #2a2f3a;font-size:12px"><span class="clr-muted">'+new Date(e.date).toLocaleDateString()+'</span> '+safe(e.event)+(e.details?' — '+safe(e.details):'')+'</div>';
      });
      html+='</div>';
    }
    ...
    if(canWrite){
      ...
      html+='<button class="small" data-pqa="'+safe(m.name)+'" data-action="note" class="m-0">📝 Add Note</button>';
      html+='<button class="small" data-pqa="'+safe(m.name)+'" data-action="watchlist" class="m-0">👁 Watchlist</button>';
  ...
  _compareInput.addEventListener('input',function(){
    ...
    _compareDD.innerHTML=matches.map(function(m){
      return'<div data-cmpname="'+safe(m.name)+'" style="padding:8px 12px;cursor:pointer;font-size:13px;border-bottom:1px solid #2a2f3a;color:#ccc;transition:background .1s"><b>'+safe(m.name)+'</b> <span class="clr-muted text-11">'+safe(guildName(m.guildId))+'</span></div>';
    }).join('');
  ...
  function renderCompareResult(ms){
    ...
    _compareCache=ms;
    if(ms.length<2){el.innerHTML='<span class="clr-orange">Need at least 2 found members.</span>';return;}
...
export function renderIdleonAdminTab(userTier) {
  ...
  const TIER_LEVELS = { owner: 4, admin: 3, moderator: 2, viewer: 1 };
  const canWrite = TIER_LEVELS[userTier] >= TIER_LEVELS.admin;
  if (!canWrite) return '<div class="card"><p class="clr-red">🔒 Admin access required.</p></div>';
  return `
<style>
...
<div class="card" class="pos-rel">
  <h2 class="m-0">🛠️ IdleOn Guild Admin</h2>
...
<div id="idlAdminFirebase" class="idl-admin-panel">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px;margin-bottom:12px">
    <div class="card" style="padding:12px 14px;margin:0">
      ...
      <div id="fbDisconnectSection" style="display:none;margin-top:8px;padding-top:8px;border-top:1px solid #3a3a42">
        <span class="text-11 clr-muted">Connected <span id="fbConnectedAt"></span></span>
        <span id="fbEmail" class="d-none"></span>
  ...
  <div id="fbAuthSection" class="card" class="d-none">
    <h3>🔐 Connect Google Account</h3>
    <p class="text-12 clr-muted">Uses the Device Code flow — you'll get a code to enter at google.com/device. Only <code>email profile</code> scope is requested. Your token is encrypted at rest.</p>
    ...
    <div id="fbAuthProgress" style="display:none;margin-top:12px;background:#1a1a2e;border:1px solid #3a3a42;border-radius:8px;padding:16px;text-align:center">
      ...
      <p class="text-12 clr-muted">Waiting for you to complete login...</p>
    </div>
...
<div id="idlAdminConfig" class="idl-admin-panel" class="d-none">
  <div class="card">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px">
      <div>
        ...
        <p class="hint-text">Days of inactivity before a member gets a warning status (yellow ➜ orange).</p>
        <input type="number" id="idlCfgWarnDays" min="1" max="60" class="m-0 w-full">
      </div>
      <div>
        <label>Kick Threshold Days (red)</label>
        <p class="hint-text">Days of inactivity before a member enters the kick queue (red status).</p>
        <input type="number" id="idlCfgKickDays" min="1" max="90" class="m-0 w-full">
      </div>
      <div>
        <label>Min Weekly GP</label>
        <p class="hint-text">Minimum guild points a member should earn per week. Used for at-risk detection.</p>
        <input type="number" id="idlCfgMinGp" min="0" class="m-0 w-full">
      </div>
      <div>
        <label>Probation Duration (weeks)</label>
        <p class="hint-text">How many weeks new members stay on probation before becoming full members.</p>
        <input type="number" id="idlCfgProbWeeks" min="1" max="12" class="m-0 w-full">
      </div>
      <div>
        <label>Probation Min GP</label>
        <p class="hint-text">Minimum total GP a member must earn during probation to pass.</p>
        <input type="number" id="idlCfgProbGp" min="0" class="m-0 w-full">
      </div>
      <div>
        <label>Warning DMs Enabled</label>
        <p class="hint-text">Send automatic Discord DMs to members nearing the kick threshold.</p>
        <select id="idlCfgWarnDms" class="m-0 w-full"><option value="false">Off</option><option value="true">On</option></select>
      </div>
      <div>
        <label>Weekly Digest Channel ID</label>
        <p class="hint-text">Discord channel where the weekly activity summary is posted.</p>
        <input type="text" id="idlCfgDigestCh" placeholder="Channel ID" class="m-0 w-full">
      </div>
      <div>
        <label>Digest Day</label>
        <p class="hint-text">Day of the week the digest is automatically posted.</p>
        <select id="idlCfgDigestDay" class="m-0 w-full"><option value="0">Sunday</option><option value="1">Monday</option><option value="2">Tuesday</option><option value="3">Wednesday</option><option value="4">Thursday</option><option value="5">Friday</option><option value="6">Saturday</option></select>
      </div>
      <div>
        <label>Forum Channel ID (waitlist)</label>
        <p class="hint-text">Forum channel to scan for new applicants wanting to join the guild.</p>
        <input type="text" id="idlCfgForumCh" placeholder="Channel ID" class="m-0 w-full">
      </div>
      <div>
        <label>LOA Channel ID (time off)</label>
        <p class="hint-text">Channel where members post about their leave of absence.</p>
        <input type="text" id="idlCfgLoaCh" placeholder="Channel ID" class="m-0 w-full">
      </div>
      <div>
        <label>Review Channel ID (account reviews)</label>
        <p class="hint-text">Discord channel where people request Idleon account reviews.</p>
        <input type="text" id="idlCfgReviewCh" placeholder="Channel ID" class="m-0 w-full">
      </div>
      <div>
        <label>Twitch Reward ID (review priority)</label>
        <p class="hint-text">Select the channel point reward that gives review priority.</p>
      ...
      <div>
        ...
        <p class="hint-text">Forum thread where members post "Name - Guild" to request guild promotion/transfer.</p>
        <input type="text" id="idlCfgPromoThread" placeholder="Thread ID" class="m-0 w-full">
      </div>
      <div>
        <label>Promotion Ping (long wait)</label>
        <p class="hint-text">Enable pinging the configured channel when someone has been waiting longer than the threshold.</p>
        <select id="idlCfgPromoPing" class="m-0 w-full"><option value="false">Off</option><option value="true">On</option></select>
      </div>
      <div>
        <label>Promotion Ping After (hours)</label>
        <p class="hint-text">How many hours before a long-wait warning appears. Default: 48.</p>
        <input type="number" id="idlCfgPromoPingHrs" value="48" min="1" max="720" class="m-0 w-full">
      </div>
      <div>
        <label>Promotion Ping Channel ID</label>
        <p class="hint-text">Discord channel to ping guild leaders when someone waits too long for promotion.</p>
        <input type="text" id="idlCfgPromoPingCh" placeholder="Channel ID" class="m-0 w-full">
      </div>
    </div>
    <div style="display:flex;gap:8px;align-items:center;margin-top:12px;flex-wrap:wrap">
      ...
      <input type="file" id="idlCfgImportFile" accept=".json" class="d-none">
      <button class="small danger" id="idlCfgReset" class="m-0">🔄 Reset to Defaults</button>
  ...
  <div class="card">
    ...
    <p class="clr-muted text-12">Override warning/kick thresholds and min GP for specific guilds. Leave blank to use global defaults.</p>
  ...
  <div class="card">
    <h2>📢 Manual Digest</h2>
    <p class="clr-muted">Send weekly digest to the configured channel now.</p>
...
<div id="idlAdminBackup" class="idl-admin-panel" class="d-none">
  <div class="card">
    <h2>💾 Data Backup & Restore</h2>
    <p class="clr-muted">Create snapshots of all IdleOn data, or restore from a previous backup.</p>
  ...
  <div class="card">
    ...
    <p class="clr-muted">Scan for data anomalies: duplicate names, orphaned guild references, missing dates.</p>
  ...
  <div class="card">
    ...
    <p class="clr-muted">Download members as CSV or JSON.</p>
    <div class="d-flex gap-4">
...
(function(){
  ...
  if(_btnCreateReward) _btnCreateReward.addEventListener('click',function(){
    ...
    fetch('/api/idleon/twitch-rewards/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:title,cost:cost})}).then(function(r){return r.json()}).then(function(d){
      ...
      status.innerHTML='<span class="clr-green">✅ Created "'+d.reward.title+'" ('+d.reward.cost+' pts) — auto-saved as reward ID</span>';
  ...
  if(_btnFetchRewards) _btnFetchRewards.addEventListener('click',function(){
    ...
    fetch('/api/idleon/twitch-rewards').then(function(r){return r.json()}).then(function(d){
      ...
      status.innerHTML='<span class="clr-green">✅ Found '+(d.rewards||[]).length+' reward'+(d.rewards.length!==1?'s':'')+'</span>';
    }).catch(function(e){status.textContent='❌ '+e.message;status.style.color='#f44336';});
  });
  ...
  function renderGuilds(){
    ...
    el.innerHTML=(model.guilds||[]).map(function(g){
      ...
      return'<div style="display:flex;align-items:center;gap:8px;padding:8px;border-bottom:1px solid #2a2f3a"><span class="flex-1"><b>'+safe(g.name)+'</b> <span class="clr-muted">('+count+' members, id: '+safe(g.id)+')</span></span><button class="small danger" data-delguild="'+safe(g.id)+'" style="margin:0;padding:2px 8px;font-size:11px">🗑️</button></div>';
  ...
  function renderKickQueue(){
    ...
    fetch('/api/idleon/kick-candidates?count='+slots).then(function(r){return r.json()}).then(function(d){
      ...
      el.innerHTML=(d.candidates||[]).map(function(c,i){
        return'<tr><td>'+(i+1)+'</td><td>'+safe(c.name)+'</td><td>'+safe(guildName(c.guildId))+'</td><td>'+c.daysAway+'d</td><td>'+c.kickRiskScore+'</td><td>'+fmtN(c.allTimeGp)+'</td><td class="text-12 clr-muted">'+safe(c.reason||'')+'</td></tr>';
      }).join('')||'<tr><td colspan="7" class="text-center clr-muted">No kick candidates</td></tr>';
  ...
  function renderWaitlist(){
    ...
    el.innerHTML=wl.map(function(w,i){
      ...
      var statusLabel=inGuild?'<span style="color:#4caf50;font-weight:600">✅ In Guild</span>':w.status==='confirmed'?'<span style="color:#2196f3">✔ Confirmed</span>':'<span class="clr-orange">⏳ Waiting</span>';
      ...
      return'<tr'+(inGuild?' style="opacity:0.5"':'')+'><td>'+(i+1)+'</td><td>'+safe(w.name)+'</td><td>'+new Date(w.addedAt).toLocaleDateString('en-US',{month:'short',day:'numeric'})+' <span class="clr-muted text-11">('+waitStr+')</span></td><td>'+safe(w.notes||'-')+'</td><td>'+safe(w.priority||'normal')+'</td><td>'+statusLabel+'</td><td class="nowrap">'+(w.status!=='confirmed'&&!inGuild?'<button class="small" data-confirmwait="'+safe(w.id)+'" style="margin:0;padding:2px 6px;font-size:11px;background:#2196f3" title="Manually confirm recruited">✅</button> ':'')+'<button class="small danger" data-delwait="'+safe(w.id)+'" style="margin:0;padding:2px 6px;font-size:11px">🗑️</button></td></tr>';
    }).join('')||'<tr><td colspan="7" class="text-center clr-muted">Waitlist empty. Scan forum or add manually.</td></tr>';
    // Auto-check status label
    var statusEl=document.getElementById('idlWaitAutoStatus');
    if(statusEl)statusEl.innerHTML='<span class="clr-green">🔄 Auto-check: on (15s)</span>';
      ...
      _waitlistInterval=setInterval(function(){
        ...
        fetch('/api/idleon/scan-forum',{method:'POST'}).then(function(r){return r.json()}).then(function(scanResult){
          ...
          if(statusEl&&scanResult.success&&scanResult.added&&scanResult.added.length>0){
            statusEl.innerHTML='<span class="clr-green">\u2705 Auto-scan found '+scanResult.added.length+' new</span>';
          }
  ...
  function renderPromotionList(){
    ...
    el.innerHTML=pl.map(function(p,i){
      ...
      var waitStr=waitHrs>=24?Math.floor(waitHrs/24)+'d '+waitHrs%24+'h':waitHrs+'h';
      var statusLabel=p.status==='confirmed'?'<span class="clr-green">✔ Done</span>':'<span class="clr-orange">⏳ Waiting</span>';
      ...
      return'<tr'+(isLongWait?' style="background:#ff980015"':'')+'><td>'+(i+1)+'</td><td>'+safe(p.name)+'</td><td>'+safe(p.targetGuild||'-')+'</td><td>'+new Date(p.addedAt).toLocaleDateString('en-US',{month:'short',day:'numeric'})+' <span class="clr-muted text-11">('+waitStr+')</span>'+(isLongWait?' <span style="color:#f44336;font-size:10px">⚠ long wait</span>':'')+'</td><td>'+safe(p.notes||'-')+'</td><td>'+statusLabel+'</td><td class="nowrap">'+(p.status!=='confirmed'?'<button class="small" data-confirmpromo="'+safe(p.id)+'" style="margin:0;padding:2px 6px;font-size:11px;background:#2196f3" title="Confirm promoted">✅</button> ':'')+'<button class="small danger" data-delpromo="'+safe(p.id)+'" style="margin:0;padding:2px 6px;font-size:11px">🗑️</button></td></tr>';
    }).join('')||'<tr><td colspan="7" class="text-center clr-muted">Promotion list empty. Scan thread or add manually.</td></tr>';
  }
  ...
  function renderRoles(){
    ...
    el.innerHTML=milestones.map(function(r){
      return'<div style="display:flex;align-items:center;gap:8px;padding:6px;border-bottom:1px solid #2a2f3a"><span class="flex-1">🏅 <b>'+fmtN(r.gpThreshold)+' GP</b> → '+safe(r.roleName||r.roleId)+' <span class="clr-muted text-11">('+safe(r.roleId)+')</span></span><button class="small danger" onclick="idlRemoveRole('+r.gpThreshold+')" style="margin:0;padding:2px 8px;font-size:11px">🗑️</button></div>';
  ...
  function renderGhostStats(){
    ...
    ].map(function(x){return '<div style="background:#1a1a22;border:1px solid #2a2f3a;border-radius:8px;padding:6px 14px;text-align:center"><div style="font-size:18px;font-weight:700;color:'+x.c+'">'+x.n+'</div><div class="text-10 clr-muted uppercase lsp-05">'+x.l+'</div></div>';}).join('');
  }
  ...
  function renderGhostContent(){
    ...
    if(ghostTab==='unlinked'){
      ...
      el.innerHTML='<div style="font-size:11px;color:#666;margin-bottom:6px">Showing '+items.length+' unlinked player'+(items.length>1?'s':'')+'</div>'
        ...
        +'<th class="th-cell">IdleOn Name</th>'
        +'<th class="th-cell">Guild</th>'
        +'<th class="th-cell">Best Matches</th>'
        ...
        +items.map(function(n){
          ...
          var sugHtml=sugs.length?sugs.map(function(s){
            return '<div style="display:flex;align-items:center;gap:4px;margin:1px 0">'
              ...
              +'<span class="font-semibold">'+safe(s.displayName||s.username)+'</span>'
              +(s.username&&s.displayName&&s.username!==s.displayName?' <span class="clr-muted text-10">('+safe(s.username)+')</span>':'')
              ...
              +'</div>';
          }).join(''):'<span class="clr-mid" style="color:#555">No matches found</span>';

          var guildLabel=n.guildId?safe(guildName(n.guildId)):'<span class="clr-mid" style="color:#555">\\u2014</span>';
      ...
      el.innerHTML='<div style="font-size:11px;color:#666;margin-bottom:6px">Showing '+ghosts.length+' Discord ghost'+(ghosts.length>1?'s':'')+'</div>'
        ...
        +'<table style="width:100%;border-collapse:collapse;font-size:12px;margin:0">'
        +'<thead><tr style="border-bottom:2px solid #2a2f3a">'
        +'<th class="th-cell">Discord User</th>'
  ...
  function loadGhosts(){
    ...
    fetch('/api/idleon/ghosts').then(function(r){return r.json()}).then(function(d){
      ...
      renderGhostStats();
      renderGhostContent();
    }).catch(function(e){el.innerHTML='<span class="clr-red2">Error: '+safe(e.message)+'</span>';});
  }
  ...
  function renderKickLog(){
    ...
    el.innerHTML=paged.map(function(l,i){
      return'<div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid #2a2f3a">'
        +'<div class="flex-1"><b>'+safe(l.memberName)+'</b> — '+(l.guildId?safe(guildName(l.guildId))+' — ':'')+safe(l.reason||'No reason')
        +' <span class="clr-muted text-11">'+new Date(l.date).toLocaleString()+' by '+safe(l.kickedBy||'?')+'</span></div>'
        ...
        +'</div>';
    }).join('')||'<div class="clr-muted">No kicks recorded.</div>';
  ...
  function renderLogStats(){
    ...
    fetch('/api/idleon/kick-stats').then(function(r){return r.json()}).then(function(d){
      ...
      el.innerHTML='<div class="idl-kpi"><div class="label">Total Kicks</div><div class="val">'+d.total+'</div></div>'
        ...
        +'<div class="idl-kpi"><div class="label">Top Reason</div><div class="val" class="text-14">'+safe((d.topReasons&&d.topReasons[0]&&d.topReasons[0].reason)||'N/A')+'</div></div>';
    }).catch(function(){});
  }
  ...
  function renderGuildOverrides(){
    ...
    var guilds=model.guilds||[];
    if(!guilds.length){el.innerHTML='<div class="clr-muted">No guilds tracked yet.</div>';return;}
    el.innerHTML=guilds.map(function(g){
      ...
      return'<div style="border:1px solid #3a3a42;border-radius:8px;padding:10px;margin-bottom:8px;background:#17171b">'
        +'<b>'+safe(g.name)+'</b> <span class="clr-muted text-11">'+safe(g.id)+'</span>'
        +'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:8px;margin-top:8px">'
        +'<div><label class="text-11">Warning Days</label><input type="number" data-ov-guild="'+safe(g.id)+'" data-ov-key="warningDays" value="'+(ov.warningDays||'')+'" placeholder="global" style="margin:0;width:100%;font-size:12px"></div>'
        +'<div><label class="text-11">Kick Days</label><input type="number" data-ov-guild="'+safe(g.id)+'" data-ov-key="kickThresholdDays" value="'+(ov.kickThresholdDays||'')+'" placeholder="global" style="margin:0;width:100%;font-size:12px"></div>'
        +'<div><label class="text-11">Min Weekly GP</label><input type="number" data-ov-guild="'+safe(g.id)+'" data-ov-key="minWeeklyGp" value="'+(ov.minWeeklyGp||'')+'" placeholder="global" style="margin:0;width:100%;font-size:12px"></div>'
        +'</div></div>';
    }).join('');
  ...
  function renderBackupList(){
    ...
    el.innerHTML='<span class="clr-muted">Loading...</span>';
    fetch('/api/idleon/backups').then(function(r){return r.json()}).then(function(d){
      if(!d.success||!d.backups||!d.backups.length){el.innerHTML='<div class="clr-muted">No backups found.</div>';return;}
      el.innerHTML=d.backups.map(function(b){
        return'<div style="display:flex;align-items:center;gap:8px;padding:8px;border-bottom:1px solid #2a2f3a">'
          +'<span class="flex-1"><b>'+safe(b.name||b)+'</b></span>'
  ...
  _on('idlAutoKickPreviewBtn','click',function(){
    ...
    fetch('/api/idleon/auto-kick-status').then(function(r){return r.json()}).then(function(d){
      if(!d.success){el.innerHTML='<span class="clr-red2">❌ '+(d.error||'Failed')+'</span>';return;}
      if(!d.atRisk.length){el.innerHTML='<span class="clr-green">✅ No members at risk (threshold: '+d.minRisk+')</span>';return;}
      ...
      fbAuthPollTimer=setInterval(function(){
        fetch('/api/idleon/firebase/check-auth').then(function(r){return r.json()}).then(function(c){
          if(c.status==='success'){
            ...
            document.getElementById('fbAuthResult').innerHTML='<span class="clr-green">✅ Connected as '+safe(c.email)+'</span>';
            btn.disabled=false;
            loadFirebaseStatus();
          } else if(c.status==='error'){
            clearInterval(fbAuthPollTimer);fbAuthPollTimer=null;
            document.getElementById('fbAuthProgress').style.display='none';
            document.getElementById('fbAuthResult').innerHTML='<span class="clr-red2">❌ '+safe(c.error)+'</span>';
            btn.disabled=false;
          }
        }).catch(function(){});
      },3000);
    }).catch(function(e){document.getElementById('fbAuthResult').innerHTML='<span class="clr-red2">❌ '+safe(e.message)+'</span>';btn.disabled=false;});
  });
  ...
  _on('fbSearchBtn','click',function(){
    ...
    fetch('/api/idleon/firebase/search-guild',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name})}).then(function(r){return r.json()}).then(function(d){
      ...
      el.innerHTML=d.guilds.map(function(g){
        ...
        return '<div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px solid #2a2f3a">' +
          '<strong>'+safe(g.name)+'</strong> <span class="clr-muted text-12">ID: '+safe(g.id)+'</span> ' +
    ...
    }).catch(function(e){el.innerHTML='<span class="clr-red2">❌ '+safe(e.message)+'</span>';});
  });
  _on('fbRefreshNow','click',function(){
    ...
    fetch('/api/idleon/firebase/refresh',{method:'POST'}).then(function(r){return r.json()}).then(function(d){
      if(!d.success)throw new Error(d.error||'Failed');
      var html='<span class="clr-green">✅ Polled at '+new Date(d.polledAt).toLocaleTimeString()+'</span><br>';
  ...
  _on('idlSyncRolesDry','click',function(){
    ...
    el.innerHTML='<span class="clr-orange">Running dry run...</span>';
    fetch('/api/idleon/sync-roles-dry',{method:'POST'}).then(function(r){return r.json()}).then(function(d){
      if(!d.success){el.innerHTML='<span class="clr-red2">❌ '+(d.error||'Failed')+'</span>';return;}
      if(!d.changes||!d.changes.length){el.innerHTML='<span class="clr-green">✅ No changes needed — all roles are up to date.</span>';return;}
      el.innerHTML='<b>'+d.changes.length+' changes would be made:</b><div style="max-height:200px;overflow-y:auto;margin-top:6px;font-size:12px">'+d.changes.map(function(c){
        return'<div style="padding:3px 0;border-bottom:1px solid #2a2f3a">'+(c.action==='add'?'<span class="clr-green">+ Add</span>':'<span class="clr-red2">- Remove</span>')+' <b>'+safe(c.roleName)+'</b> → '+safe(c.memberName)+'</div>';
      }).join('')+'</div>';
    }).catch(function(e){el.innerHTML='<span class="clr-red2">❌ '+e.message+'</span>';});
  });
  ...
  _on('idlBackupCreate','click',function(){
    ...
    fetch('/api/idleon/backup',{method:'POST'}).then(function(r){return r.json()}).then(function(d){
      el.innerHTML=d.success?'<span class="clr-green">✅ Backup created: '+safe(d.name||'')+'</span>':'<span class="clr-red2">❌ '+(d.error||'Failed')+'</span>';
      renderBackupList();
    }).catch(function(e){el.innerHTML='<span class="clr-red2">❌ '+e.message+'</span>';});
  });
  ...
  _on('idlIntegrityRun','click',function(){
    var el=document.getElementById('idlIntegrityResult');el.innerHTML='<span class="clr-orange">Checking...</span>';
    fetch('/api/idleon/integrity').then(function(r){return r.json()}).then(function(d){
      if(!d.success){el.innerHTML='<span class="clr-red2">❌ '+(d.error||'Failed')+'</span>';return;}
      var issues=d.issues||[];
      if(!issues.length){el.innerHTML='<span class="clr-green">✅ No issues found. Data integrity OK.</span>';return;}
      el.innerHTML='<b class="clr-orange">⚠️ '+issues.length+' issues found:</b><div style="max-height:200px;overflow-y:auto;margin-top:6px">'+issues.map(function(is){
        return'<div style="padding:4px 0;border-bottom:1px solid #2a2f3a;font-size:12px"><span class="clr-orange">'+safe(is.type)+'</span> — '+safe(is.detail)+'</div>';
      }).join('')+'</div>';
    }).catch(function(e){el.innerHTML='<span class="clr-red2">❌ '+e.message+'</span>';});
  });
...
<div class="card">
  ...
  <p class="clr-muted">Kick queue, recruitment waitlist, promotions, auto-kick system, roles &amp; member linking.</p>
</div>
...
<div id="gmKicks" class="gm-panel">
  <div class="card">
    ...
    <p class="clr-muted">Members ranked by kick priority. LOA and exempt members excluded.</p>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">
      <label class="text-13">Free up <input type="number" id="gmKickSlots" value="5" min="1" max="50" style="width:60px;margin:0"> slots</label>
      <button class="small" id="gmKickRefresh" class="m-0">🔄 Refresh</button>
    ...
    <div style="border:1px solid #3a3a42;border-radius:8px;background:#17171b">
      <table class="m-0"><thead><tr><th style="width:60px">Priority</th><th>Member</th><th>Guild</th><th style="width:90px">Days Away</th><th>Risk</th><th>GP</th><th>Reason</th></tr></thead>
      <tbody id="gmKickRows"><tr><td colspan="7" class="text-center clr-muted">Loading...</td></tr></tbody></table>
    </div>
  ...
  <div class="card">
    ...
    <p class="clr-muted">People waiting to join. Scan forum channel or add manually.</p>
    ...
    <div id="gmWaitListWrap" class="gm-scroll-list" style="border:1px solid #3a3a42;border-radius:8px;background:#17171b">
      <table class="m-0"><thead><tr><th>#</th><th>Name</th><th>Added</th><th>Notes</th><th>Priority</th><th>Status</th><th>Actions</th></tr></thead>
  ...
  <div class="card">
    ...
    <p class="clr-muted">People wanting to move to a different guild. Scan the promotion thread or add manually.</p>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px;align-items:center">
    ...
    <div id="gmPromoListWrap" class="gm-scroll-list" style="border:1px solid #3a3a42;border-radius:8px;background:#17171b">
      <table class="m-0"><thead><tr><th>#</th><th>Name</th><th>Current Guild</th><th>Target Guild</th><th>Status</th><th>Actions</th></tr></thead>
...
<!-- Roles & Links Panel -->
<div id="gmRoles" class="gm-panel" class="d-none">
  <div class="card">
    <h2>🏅 GP Role Milestones</h2>
    <p class="clr-muted">Define GP thresholds that auto-assign Discord roles. Members must be linked to their Discord account.</p>
  ...
  <div class="card">
    ...
    <p class="clr-muted">Cross-reference IdleOn members with Discord server members to find mismatches. Fuzzy matching detects similar names.</p>
...
(function(){
  ...
  function renderKickQueue(){
    ...
    fetch('/api/idleon/kick-candidates?count='+Math.max(slots,5)).then(function(r){return r.json()}).then(function(d){
      if(!d.success)return;
      el.innerHTML=(d.candidates||[]).map(function(c,i){
        return'<tr><td>'+(i+1)+'</td><td><a href="#" data-gm-player="'+safe(c.name)+'" style="color:#4fc3f7;text-decoration:none;cursor:pointer">'+safe(c.name)+'</a></td><td>'+safe(guildName(c.guildId))+'</td><td>'+c.daysAway+'d</td><td>'+c.kickRiskScore+'</td><td>'+fmtN(c.allTimeGp)+'</td><td class="text-12 clr-muted">'+safe(c.reason||'')+'</td></tr>';
      }).join('')||'<tr><td colspan="7" class="text-center clr-muted">No kick candidates</td></tr>';
      ...
      var ks=document.getElementById('gmKickAutoStatus');if(ks)ks.innerHTML='<span class="clr-green">✅ Last refreshed: '+new Date().toLocaleTimeString()+'</span>';
    }).catch(function(){});
  }
  ...
  function renderPromotionList(){
    ...
    el.innerHTML=pl.map(function(p,i){
      ...
      var statusLabel=isDone?'<span class="gm-done-badge">✅ Done</span>':'<span class="clr-orange">⏳ Waiting</span>';
      return'<tr'+(isDone?' style="opacity:0.45"':'')+'><td>'+(i+1)+'</td><td>'+safe(p.name)+'</td><td>'+safe(curGuild)+'</td><td>'+safe(p.targetGuild||'-')+'</td><td>'+statusLabel+'</td><td class="nowrap">'+(isDone?'':'<button class="small" data-gm-confirmpromo="'+safe(p.id)+'" style="margin:0;padding:2px 6px;font-size:11px;background:#2196f3" title="Confirm promoted">✅</button> ')+'<button class="small danger" data-gm-delpromo="'+safe(p.id)+'" style="margin:0;padding:2px 6px;font-size:11px">🗑️</button></td></tr>';
    }).join('')||'<tr><td colspan="6" class="text-center clr-muted">Promotion list empty. Scan thread or add manually.</td></tr>';
    ...
    if(promoStatus)promoStatus.innerHTML='<span class="clr-green">🔄 Auto-check: on (15s)</span>';
    if(!_promoInterval){
  ...
  function renderGhostContent(){
    ...
    if(ghostTab==='unlinked'){
      ...
      var allDiscord=ghostData.allDiscord||[];
      el.innerHTML='<div style="font-size:11px;color:#666;margin-bottom:6px">Showing '+items.length+' unlinked player'+(items.length>1?'s':'')+'</div><div style="border:1px solid #2a2f3a;border-radius:8px;overflow:hidden;background:#17171b"><table style="width:100%;border-collapse:collapse;font-size:12px;margin:0"><thead><tr style="border-bottom:2px solid #2a2f3a"><th class="th-cell">IdleOn Name</th><th class="th-cell">Guild</th><th class="th-cell">Best Matches</th><th style="padding:8px 10px;text-align:left;color:#8b8fa3;font-size:10px;text-transform:uppercase;width:220px">Manual Link</th><th style="padding:8px 10px;text-align:center;color:#8b8fa3;font-size:10px;text-transform:uppercase;width:80px">Actions</th></tr></thead><tbody>'+items.map(function(n){
        var sugs=n.suggestions||[];
        var sugHtml=sugs.length?sugs.map(function(s){return '<div style="display:flex;align-items:center;gap:4px;margin:1px 0"><span style="background:'+scoreColor(s.score)+'22;color:'+scoreColor(s.score)+';padding:1px 6px;border-radius:10px;font-size:9px;font-weight:700;min-width:28px;text-align:center">'+s.score+'%</span><span class="font-semibold">'+safe(s.displayName||s.username)+'</span>'+(s.username&&s.displayName&&s.username!==s.displayName?' <span class="clr-muted text-10">('+safe(s.username)+')</span>':'')+'<button class="small" data-gm-ghost-link="'+safe(n.idleonName)+'" data-gm-ghost-did="'+safe(s.id)+'" style="margin:0;padding:1px 6px;font-size:9px;background:#4caf5022;color:#4caf50;border:1px solid #4caf5044" title="Link">\\u2714 Link</button></div>';}).join(''):'<span class="clr-mid" style="color:#555">No matches found</span>';
        var guildLabel=n.guildId?safe(guildName(n.guildId)):'<span class="clr-mid" style="color:#555">\\u2014</span>';
      ...
      el.innerHTML='<div style="font-size:11px;color:#666;margin-bottom:6px">Showing '+ghosts.length+' Discord ghost'+(ghosts.length>1?'s':'')+'</div><div style="border:1px solid #2a2f3a;border-radius:8px;overflow:hidden;background:#17171b"><table style="width:100%;border-collapse:collapse;font-size:12px;margin:0"><thead><tr style="border-bottom:2px solid #2a2f3a"><th class="th-cell">Discord User</th><th class="th-cell">Username</th><th class="th-cell">User ID</th><th style="padding:8px 10px;text-align:center;color:#8b8fa3;font-size:10px;text-transform:uppercase;width:80px">Actions</th></tr></thead><tbody>'+ghosts.map(function(g){
  ...
  function openPlayerModal(name){
    ...
    html+='<div style="background:#0e0e12;border:1px solid #2a2f3a;border-radius:8px;padding:6px 12px;text-align:center;flex:1;min-width:80px"><div class="text-10 clr-muted uppercase">Guild</div><div style="font-size:14px;font-weight:600;color:#4fc3f7">'+safe(guild)+'</div></div>';
    html+='<div style="background:#0e0e12;border:1px solid #2a2f3a;border-radius:8px;padding:6px 12px;text-align:center;flex:1;min-width:80px"><div class="text-10 clr-muted uppercase">Status</div><div style="font-size:14px;font-weight:600;color:'+(statusColors[status]||'#ccc')+'">'+safe(status)+'</div></div>';
    html+='<div style="background:#0e0e12;border:1px solid #2a2f3a;border-radius:8px;padding:6px 12px;text-align:center;flex:1;min-width:80px"><div class="text-10 clr-muted uppercase">Risk</div><div style="font-size:14px;font-weight:600;color:'+(risk>=70?'#f44336':risk>=40?'#ff9800':'#4caf50')+'">'+risk+'</div></div>';
    html+='</div>';
    html+='<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px">';
    html+='<div style="background:#0e0e12;border:1px solid #2a2f3a;border-radius:8px;padding:6px 12px;text-align:center;flex:1;min-width:80px"><div class="text-10 clr-muted uppercase">All-Time GP</div><div style="font-size:14px;font-weight:600;color:#e0e0e0">'+fmtN(atGp)+'</div></div>';
    html+='<div style="background:#0e0e12;border:1px solid #2a2f3a;border-radius:8px;padding:6px 12px;text-align:center;flex:1;min-width:80px"><div class="text-10 clr-muted uppercase">Weekly GP</div><div style="font-size:14px;font-weight:600;color:#e0e0e0">'+fmtN(wGp)+'</div></div>';
    html+='<div style="background:#0e0e12;border:1px solid #2a2f3a;border-radius:8px;padding:6px 12px;text-align:center;flex:1;min-width:80px"><div class="text-10 clr-muted uppercase">Days Away</div><div style="font-size:14px;font-weight:600;color:'+(dAway>=14?'#f44336':dAway>=7?'#ff9800':'#4caf50')+'">'+dAway+'</div></div>';
    html+='</div>';
  ...
  (function(){
    ...
    searchInput.addEventListener('input',function(){
      ...
      debounce=setTimeout(function(){
        ...
        if(!matches.length){
          resultEl.innerHTML='<span class="clr-green">✅ Not found in any guild</span>';
        }else{
          resultEl.innerHTML=matches.map(function(m){
            return'<div style="padding:2px 0"><span class="clr-red2">⚠</span> <b>'+safe(m.name)+'</b> <span class="clr-muted">— '+safe(guildName(m.guildId))+'</span></div>';
          }).join('');
        }
  ...
  document.getElementById('gmSyncRolesDry').addEventListener('click',function(){
    ...
    fetch('/api/idleon/sync-roles-dry',{method:'POST'}).then(function(r){return r.json()}).then(function(d){
      if(!d.success){el.innerHTML='<span class="clr-red2">❌ '+(d.error||'Failed')+'</span>';return;}
      if(!d.changes||!d.changes.length){el.innerHTML='<span class="clr-green">✅ No changes needed.</span>';return;}
      el.innerHTML='<b>'+d.changes.length+' changes would be made:</b><div style="max-height:200px;overflow-y:auto;margin-top:6px;font-size:12px">'+d.changes.map(function(c){return'<div style="padding:3px 0;border-bottom:1px solid #2a2f3a">'+(c.action==='add'?'<span class="clr-green">+ Add</span>':'<span class="clr-red2">- Remove</span>')+' <b>'+safe(c.roleName)+'</b> → '+safe(c.memberName)+'</div>';}).join('')+'</div>';
    }).catch(function(e){el.innerHTML='<span class="clr-red2">❌ '+e.message+'</span>';});
  });
...
<div class="rv-wrap">
  ...
  <div id="rvMyReviewPanel" style="background:linear-gradient(135deg,var(--bg2),var(--bg1));border:1px solid var(--brd);border-radius:var(--r-m);padding:10px 14px;margin-bottom:8px">
    ...
    <div class="d-flex gap-3 items-center flex-wrap">
  ...
  <div class="rv-stats" id="rvStatsRow">
    ...
    <div class="rv-stat"><div class="rv-stat-tip">Longest time waiting</div><div class="rv-stat-icon">&#x1F534;</div><div class="num" id="rvStatOldest" class="text-danger">-</div><div class="lbl">Oldest</div></div>
    ...
    <div class="rv-stat" data-rvfilter="stale"><div class="rv-stat-tip">Waiting 7+ days</div><div class="rv-stat-icon">&#x26A0;&#xFE0F;</div><div class="num" id="rvStatStale" class="text-danger">0</div><div class="lbl">Stale (7d+)</div><span class="rv-stat-delta" id="rvDeltaStale"></span></div>
  </div>
  ...
  <div class="rv-cat-filter" id="rvCatFilter" class="d-none">
  ...
  <div class="rv-card">
    ...
    <div id="rvQuickFilters" class="d-none"><button class="rv-qf active" data-qf="all">All <span class="rv-qf-count" id="rvQfAll">0</span></button><span id="rvQfRedeemed"></span><span id="rvQfStale"></span><span id="rvQfActive"></span><span id="rvQfPending"></span><span id="rvQfOnHold"></span><span id="rvQfToday"></span></div>
    ...
    <!-- Completed -->
    <div id="rvCompleted" class="rv-completed-section" class="d-none"></div>
...
<div class="rv-modal-overlay" id="rvImportModal">
  <div class="rv-modal">
    ...
    <div class="rv-import-zone" id="rvImportZone">
      ...
      <input type="file" id="rvImportFile" accept=".csv,.json" class="d-none">
    </div>
...
(function(){
  ...
  function renderStats(){
    ...
    if(sub){
      if(active.length===0&&done.length===0){
        ...
        sub.innerHTML='<span class="rv-hl">'+active.length+'</span> in queue'+(redeemed.length?' (<span class="rv-hl-warn">'+redeemed.length+' redeemed</span>)':'')+(stale.length?' \\u2022 <span class="text-danger">'+stale.length+' stale</span>':'');
      }
    }
  }
  /* === Quick filter counts === */
  ...
  document.getElementById('rvRows').addEventListener('click',function(e){
    ...
    if(editBtn){
      ...
      cell.innerHTML='<div class="rv-edit-form">'
        ...
        +'<input id="rvEditName-'+safe(rid)+'" value="'+safe(rv.name)+'" placeholder="Who is the review for?" class="font-semibold">'
  ...
  document.getElementById('rvSyncTwitch').addEventListener('click',function(){
    ...
    fetch('/api/idleon/account-reviews/sync-twitch',{method:'POST'}).then(function(r){return r.json()}).then(function(d){
      ...
      el.innerHTML=d.success?'<span style="color:var(--twi)">\uD83D\uDCFA +'+d.added+' added, '+d.upgraded+' upgraded ('+d.totalRedemptions+' redemptions scanned)</span>':'<span class="text-danger">\u274C '+(d.error||'Failed')+'</span>';
  ...
  document.getElementById('rvClearDone').addEventListener('click',function(){
    ...
    rvConfirm('Clear Completed Reviews','Remove all '+completed.length+' completed reviews from the queue?',
      ...
      function(){
        ...
        }).catch(function(e){document.getElementById('rvStatus').innerHTML='<span class="text-danger">\u274C '+e.message+'</span>';});
  ...
  function showUndoToast(msg){
    ...
    toast.innerHTML='<span>'+msg+'</span><button id="rvUndoBtn">Undo</button><div class="rv-undo-timer" class="w-full"></div>';
...
<div class="card">
  ...
  <p class="clr-muted">Deep analytics across all guilds: distributions, leaderboards, level predictions, and guild bonuses.</p>
</div>
...
(function(){
  ...
  function renderLeaderboard(el,items,valFn){
    el.innerHTML=items.map(function(m,i){
      ...
      return'<div class="idl-lb-item"><span class="idl-lb-rank" style="color:'+(i<3?'#ff9800':'#8b8fa3')+'">'+medal+'</span><span class="flex-1"><b>'+safe(m.name)+'</b></span><span class="clr-muted text-12">'+valFn(m)+'</span></div>';
    }).join('')||'<div class="clr-muted">No data</div>';
  }
  ...
  Promise.all([
    ...
    document.getElementById('idlStatsLevelDist').innerHTML=Object.entries(ld).map(function(e){return renderBar(e[0],e[1],lMax,lColors[e[0]]||'#777')}).join('')||'<div class="clr-muted">No level data (connect Firebase)</div>';
    ...
    document.getElementById('idlStatsStreaks').innerHTML=
      ...
      +'<div class="idl-stat-row"><span>Max Streak</span><span class="clr-orange">🔥 '+ss.max+' weeks</span></div>'
      +'<div class="idl-stat-row"><span>Zero Streaks</span><span class="clr-red2">'+ss.zeroCount+' members</span></div>';
    ...
    if(gi.success&&gi.guilds.length){
      document.getElementById('idlStatsGuildLevels').innerHTML=gi.guilds.map(function(g){
        ...
        return'<div class="idl-guild-lv">'
          ...
          +'<div class="lv-bar">'
          +'<div style="font-weight:700;margin-bottom:4px">'+safe(g.name)+' <span class="clr-muted text-12">('+g.members+' members)</span></div>'
          +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><div style="flex:1;background:#333;border-radius:4px;height:10px;overflow:hidden"><div style="width:'+pct+'%;background:#7c3aed;height:100%;border-radius:4px"></div></div><span class="text-12 clr-muted">'+pct+'%</span></div>'
          +'<div class="text-12 clr-muted">'+fmtN(g.totalGp)+' / '+fmtN(g.nextLevelGp)+' GP · Need '+fmtN(g.gpNeeded)+' more · Avg '+fmtN(g.avgWeeklyGp)+'/wk</div>'
          +'<div style="font-size:12px;color:#ff9800;margin-top:2px">⏱️ ETA to Lv.'+(g.currentLevel+1)+': '+predTxt+'</div>'
          +(g.bonusLevels&&g.bonusLevels.length?'<div class="text-11 clr-muted mt-1">Bonus levels: '+g.bonusLevels.reduce(function(s,v){return s+v},0)+' total ('+g.bonusLevels.filter(function(v){return v>0}).length+'/'+g.bonusLevels.length+' active)</div>':'')
          +'</div></div>';
      }).join('');
    } else {
      document.getElementById('idlStatsGuildLevels').innerHTML='<div class="clr-muted">No guild data available. Connect Firebase and add guilds.</div>';
    }
    ...
    if(gi.success&&gi.guilds.length){
      ...
      document.getElementById('idlStatsBonuses').innerHTML=gi.guilds.map(function(g){
        ...
        if(!entries.length)return'<div class="idl-stat-card"><h4>'+safe(g.name)+'</h4><div class="clr-muted text-12">No bonus data</div></div>';
        return'<div class="idl-stat-card"><h4>'+safe(g.name)+' <span class="clr-muted text-11">('+g.members+' members)</span></h4>'
          +entries.map(function(e,i){return renderBar(e[0],e[1],maxB,bonusColors[i%bonusColors.length])}).join('')
          +'</div>';
      }).join('');
    } else {
      document.getElementById('idlStatsBonuses').innerHTML='<div class="clr-muted">No bonus data. Connect Firebase to fetch guild member bonuses.</div>';
    }
    ...
    if(gi.success&&gi.guilds.length){
      ...
      if(hasAny){
        document.getElementById('idlStatsBonusLevels').innerHTML=gi.guilds.map(function(g){
          ...
          if(!bl.length)return'<div class="idl-stat-card"><h4>'+safe(g.name)+'</h4><div class="clr-muted text-12">No bonus level data yet</div></div>';
        ...
        document.getElementById('idlStatsBonusLevels').innerHTML='<div class="clr-muted">Bonus levels not yet available. They will appear after the next Firebase poll.</div>';
      }
    } else {
      document.getElementById('idlStatsBonusLevels').innerHTML='<div class="clr-muted">No guild data. Connect Firebase to fetch bonus levels.</div>';
    }
    ...
    document.getElementById('idlStatsGuildCompare').innerHTML=gs.map(function(g){
      return'<div class="idl-stat-card"><h4>'+safe(g.name)+'</h4>'
        ...
        +'</div>';
    }).join('')||'<div class="clr-muted">No guilds configured.</div>';
    ...
    document.getElementById('idlStatsKicks').innerHTML=
      '<div class="idl-kpi"><div class="label">Total Kicks</div><div class="val" class="clr-red2">'+ks.total+'</div></div>'
      +'<div class="idl-kpi"><div class="label">Last 7 Days</div><div class="val" class="clr-orange">'+ks.last7d+'</div></div>'
      +'<div class="idl-kpi"><div class="label">Last 30 Days</div><div class="val" style="color:#ffc107">'+ks.last30d+'</div></div>';

  }).catch(function(e){
    console.error('Stats load error:',e);
    document.getElementById('idlStatsKpis').innerHTML='<div class="card" style="grid-column:1/-1"><p class="clr-red2">❌ Failed to load stats: '+e.message+'</p></div>';
  });
})();
</script>`;
}
...
<div class="card">
  ...
  <p class="clr-muted">Track member activity patterns, retention, and growth trends across all guilds.</p>
</div>
...
(function(){
  ...
  Promise.all([
    ...
    if(allWeeks.length>=2){
      ...
      if(impEl)impEl.innerHTML=improvers.map(function(c,i){return '<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #2a2f3a;font-size:12px"><span>'+(i+1)+'. <b>'+safe(c.name)+'</b></span><span class="clr-green">+'+fmtN(c.diff)+' GP</span></div>';}).join('')||'<div class="clr-muted text-12">No improvements this week</div>';
      var decEl=document.getElementById('idlActDecliners');
      if(decEl)decEl.innerHTML=decliners.map(function(c,i){return '<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #2a2f3a;font-size:12px"><span>'+(i+1)+'. <b>'+safe(c.name)+'</b></span><span class="clr-red2">'+fmtN(c.diff)+' GP</span></div>';}).join('')||'<div class="clr-muted text-12">No declines this week</div>';
    }
    ...
    if(fcEl){
      ...
      if(atRisk.length){
        fcEl.innerHTML='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:8px">'+atRisk.map(function(m){
          ...
          return '<div style="background:#1e1e24;border:1px solid '+color+'33;border-radius:8px;padding:8px 10px"><div style="display:flex;justify-content:space-between;margin-bottom:4px"><b class="text-13">'+safe(m.name)+'</b><span style="color:'+color+';font-size:12px;font-weight:700">Risk: '+risk+'</span></div><div class="text-11 clr-muted">Days away: '+m.daysAway+' · Weekly: '+fmtN(wGp)+' · Trend: <span style="color:'+(trend>=0?'#4caf50':'#f44336')+'">'+( trend>=0?'+':'')+fmtN(trend)+'</span></div></div>';
        }).join('')+'</div>';
      }else{
    ...
    var hmEl=document.getElementById('idlActHeatmap');
    if(hmEl)hmEl.innerHTML='<div class="clr-red2">❌ Failed to load: '+safe(e.message)+'</div>';
  });
})();
</script>`;
}
...
<div class="card">
  ...
  <p class="clr-muted">Download bot datasets as JSON or CSV.</p>
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px;margin-top:8px">
    ${exportTypes.map(t => `<div style="background:#2a2f3a;border:1px solid #3a3a42;border-radius:8px;padding:12px">
      ...
      <div class="d-flex gap-4">
        <button class="small" class="m-0" onclick="window.location.href='/api/export/${t.key}?format=json'">JSON</button>
...
<div class="card">
  <h2>💾 Data Backups</h2>
  <p class="clr-muted">Need full settings/data snapshots? Use the <a href="/backups">Backups tab</a>.</p>
</div>`;
}
...
<div class="card">
  ...
  <p class="clr-muted">Create a full snapshot of all bot JSON data, restore existing backups, or upload a backup file.</p>
  <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px">
    <button class="small" id="backupCreateBtn" class="m-0">➕ Create Backup</button>
    ...
    <label class="small" style="margin:0;background:#2a2f3a;border:1px solid #3a3a42;cursor:pointer">📤 Upload Backup
      <input id="backupUploadInput" type="file" accept="application/json,.json" class="d-none">
    </label>
  </div>
</div>
  ...
  function loadBackups(){
    fetch('/api/backups').then(function(r){ return r.json(); }).then(function(d){
      ...
      var rows = (d.backups||[]).map(function(b){
        ...
        return '<tr>'
          ...
          + '<td style="display:flex;gap:6px">'
            ...
            + '<button class="small danger" class="m-0" onclick="deleteBackup(\\\'' + b.name.replace(/\'/g, "\\\\\'") + '\\\')">Delete</button>'
          + '</td>'
        + '</tr>';
      }).join('');
      ...
      document.getElementById('backupList').innerHTML = rows
        ...
        : '<div class="clr-muted">No backups yet.</div>';
    }).catch(function(e){
      document.getElementById('backupList').innerHTML = '<div class="clr-red">'+e.message+'</div>';
    });
  }
...
<div class="card">
  ...
  <div id="accountsList" class="mt-3" style="margin-top:10px">
...
function renderPageAccessSelector() {
  ...
  root.innerHTML = pageAccessOptions.map(function(p, idx){
    ...
    return '<div style="display:grid;grid-template-columns:auto 1fr 120px;gap:8px;align-items:center;padding:6px 4px;border-bottom:1px solid #222228">'
      ...
      + '<label for="' + id + '" style="margin:0;font-size:12px;color:#d0d0d0">' + _pageLabel(p.slug) + ' <span class="clr-muted text-11">(' + p.category + ')</span></label>'
...
function loadAccounts() {
  fetch('/api/accounts').then(r=>r.json()).then(d=>{
    ...
    if(accts.length===0) { document.getElementById('accountsList').innerHTML='<div class="clr-muted">No accounts found.</div>'; return; }
    ...
    accts.forEach(function(a) {
      ...
      const chAccess = pageAccessEntries.length
        ...
        : '<span class="clr-muted">Default</span>';
      html += '<tr>';
      html += '<td class="font-semibold">' + a.username + '</td>';
      html += '<td class="clr-muted text-12">' + (a.displayName || '-') + '</td>';
      ...
      html += '<td class="text-12 clr-muted">' + lastLogin + '</td>';
      html += '<td class="text-12 clr-muted">' + created + '</td>';
...
<div class="card" style="padding:16px">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:8px">
    <div>
      <label class="sub-xs">Log Channel</label>
      <select id="auditChannelId" class="m-0">
        <option value="">Select channel...</option>
        ${alChannels.map(c => `<option value="${c.id}"${c.id === (auditLogSettings.channelId||'') ? ' selected' : ''}>#${c.name}</option>`).join('')}
      </select>
    </div>
    <div>
      <label class="sub-xs">Log Level</label>
      <select id="auditLogLevel" class="m-0">
  ...
  <div id="mlEvents" style="padding:8px;background:#1e1f22;border:1px solid #2a2f3a;border-radius:6px;margin-bottom:6px">
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;font-size:11px">
      <div style="background:#1a1a1d;padding:5px 6px;border-radius:4px">
        ...
        <label class="row-flex-sm"><input type="checkbox" id="logMemberJoins" ${auditLogSettings.logMemberJoins?'checked':''}><span>Joins & position</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logMemberLeaves" ${auditLogSettings.logMemberLeaves?'checked':''}><span>Leaves</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="warnNewAccounts" ${auditLogSettings.warnNewAccounts?'checked':''}><span>New account warn</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logMemberBoosts" ${auditLogSettings.logMemberBoosts?'checked':''}><span>Boosts</span></label>
        <div style="margin-top:4px"><label class="text-10 clr-muted">New acct threshold (days)</label><input id="newAccountThresholdDays" type="number" min="1" max="365" value="${auditLogSettings.newAccountThresholdDays || 7}" style="margin:2px 0;width:80px"></div>
      </div>
      <div style="background:#1a1a1d;padding:5px 6px;border-radius:4px">
        <div style="font-weight:600;margin-bottom:4px;font-size:10px;color:#E74C3C">🛡️ Moderation</div>
        <label class="row-flex-sm"><input type="checkbox" id="logMemberBans" ${auditLogSettings.logMemberBans?'checked':''}><span>Bans & unbans</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logMemberTimeouts" ${auditLogSettings.logMemberTimeouts?'checked':''}><span>Timeouts</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logMemberMutes" ${auditLogSettings.logMemberMutes?'checked':''}><span>Mutes</span></label>
      </div>
      <div style="background:#1a1a1d;padding:5px 6px;border-radius:4px">
        <div style="font-weight:600;margin-bottom:4px;font-size:10px;color:#1ABC9C">👤 Profile</div>
        <label class="row-flex-sm"><input type="checkbox" id="logUsernameChanges" ${auditLogSettings.logUsernameChanges?'checked':''}><span>Name changes</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logAvatarChanges" ${auditLogSettings.logAvatarChanges?'checked':''}><span>Avatar changes</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logRoleChanges" ${auditLogSettings.logRoleChanges?'checked':''}><span>Role updates</span></label>
      </div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;margin-top:4px;font-size:11px">
      <div style="background:#1a1a1d;padding:5px 6px;border-radius:4px">
        ...
        <label class="row-flex-sm"><input type="checkbox" id="logMessageEdits" ${auditLogSettings.logMessageEdits?'checked':''}><span>Edits</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logMessageDeletes" ${auditLogSettings.logMessageDeletes?'checked':''}><span>Deletes & bulk</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logMessagePins" ${auditLogSettings.logMessagePins?'checked':''}><span>Pins / unpins</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="storeDeletedImages" ${auditLogSettings.storeDeletedImages?'checked':''}><span>📸 Store deleted images</span></label>
      </div>
      <div style="background:#1a1a1d;padding:5px 6px;border-radius:4px">
        <div style="font-weight:600;margin-bottom:4px;font-size:10px;color:#7289DA">⚙️ Server</div>
        <label class="row-flex-sm"><input type="checkbox" id="logServerUpdates" ${auditLogSettings.logServerUpdates?'checked':''}><span>Settings updates</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logIntegrations" ${auditLogSettings.logIntegrations?'checked':''}><span>Bots add/remove</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logChannelChanges" ${auditLogSettings.logChannelChanges?'checked':''}><span>Channel changes</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logEmojiChanges" ${auditLogSettings.logEmojiChanges?'checked':''}><span>Emoji/sticker</span></label>
      </div>
      <div style="background:#1a1a1d;padding:5px 6px;border-radius:4px">
        <div style="font-weight:600;margin-bottom:4px;font-size:10px;color:#F47FFF">🔊 Voice & Threads</div>
        <label class="row-flex-sm"><input type="checkbox" id="logVoiceChanges" ${auditLogSettings.logVoiceChanges?'checked':''}><span>Voice join/leave/move</span></label>
        <label class="row-flex-sm"><input type="checkbox" id="logThreadChanges" ${auditLogSettings.logThreadChanges?'checked':''}><span>Thread create/archive</span></label>
      </div>
    </div>
  </div>
  ...
  <div id="mlGrouping" style="display:none;padding:8px;background:#1e1f22;border:1px solid #2a2f3a;border-radius:6px;margin-bottom:6px">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">
      ...
      <div>
        <label class="sub-xs">Group interval (seconds)</label>
  ...
  <div id="mlExclusions" style="display:none;padding:8px;background:#1e1f22;border:1px solid #2a2f3a;border-radius:6px;margin-bottom:6px">
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px">
      <div>
        <label class="sub-xs">Excluded Channels</label>
      ...
      <div>
        <label class="sub-xs">Excluded Roles</label>
      ...
      <div>
        <label class="sub-xs">Excluded Users</label>
      ...
      <div>
        <label class="sub-xs">Mute Roles <button class="small" type="button" onclick="autoDetectMuteRoles()" style="width:auto;padding:1px 6px;font-size:9px;margin:0">🔍 Detect</button></label>
  ...
  <div id="mlRetention" style="display:none;padding:8px;background:#1e1f22;border:1px solid #2a2f3a;border-radius:6px;margin-bottom:6px">
    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
      <div>
        <label class="sub-xs">Keep logs (days)</label>
  ...
  <div id="auditLogData" data-per-event-channels='${JSON.stringify(auditLogSettings.perEventChannels || {}).replace(/"/g, '&quot;')}' data-per-event-pings='${JSON.stringify(auditLogSettings.perEventPings || {}).replace(/"/g, '&quot;')}' data-event-colors='${JSON.stringify(auditLogSettings.eventColors || {}).replace(/"/g, '&quot;')}' class="d-none"></div>

  <button onclick="saveAuditLogSettings()" class="mt-2">💾 Save Member Log Settings</button>
</div>
...
function openEventColorsModal() {
  ...
  showCustomModal('<div><h3 class="mt-0">Event Colors</h3>'+rows+'</div>', () => {
    const colors = {};
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    ...
    <div class="flex-1"><span class="mc-title">Quarantine System</span><div class="mc-desc">Auto-assign quarantine role to new members</div></div>
  ...
  <div class="mc-body">
    ...
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveQuarantine()">💾 Save</button><span id="mc_quarantine_st" class="text-11"></span></div>
  </div>
</div>
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    <span class="mc-icon">📷</span>
    <div class="flex-1"><span class="mc-title">Media-Only Channels</span><div class="mc-desc">Only allow images/videos/files in chosen channels</div></div>
  ...
  <div class="mc-body">
    ...
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveMedia()">💾 Save</button><span id="mc_media_st" class="text-11"></span></div>
  </div>
</div>
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    ...
    <div class="flex-1"><span class="mc-title">Anti-Alt Detector</span><div class="mc-desc">Flag accounts younger than configurable age</div></div>
  ...
  <div class="mc-body">
    ...
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveAntiAlt()">💾 Save</button><span id="mc_antialt_st" class="text-11"></span></div>
  </div>
</div>
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    <span class="mc-icon">📝</span>
    <div class="flex-1"><span class="mc-title">Member Notes</span><div class="mc-desc">Private mod-only notes per member</div></div>
  ...
  <div class="mc-body">
    <div class="clr-muted text-11">Notes are managed per-member via Discord commands or the moderation panel.</div>
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveSimple('member-notes','mc_notes_on','mc_notes_st')">💾 Save</button><span id="mc_notes_st" class="text-11"></span></div>
  </div>
</div>
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    <span class="mc-icon">📬</span>
    <div class="flex-1"><span class="mc-title">Mod Mail</span><div class="mc-desc">DM-to-channel moderation mail system</div></div>
  ...
  <div class="mc-body">
    ...
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveModmail()">💾 Save</button><span id="mc_modmail_st" class="text-11"></span></div>
  </div>
</div>
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    <span class="mc-icon">🔖</span>
    <div class="flex-1"><span class="mc-title">Message Bookmarks</span><div class="mc-desc">React with emoji to DM the message link</div></div>
  ...
  <div class="mc-body">
    ...
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveBookmarks()">💾 Save</button><span id="mc_bookmarks_st" class="text-11"></span></div>
  </div>
</div>
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    <span class="mc-icon">🔄</span>
    <div class="flex-1"><span class="mc-title">Auto-Role on Rejoin</span><div class="mc-desc">Restore member roles when they rejoin</div></div>
  ...
  <div class="mc-body">
    <div class="clr-muted text-11">Roles are saved on leave and restored on rejoin. Toggle to enable.</div>
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveSimple('auto-role-rejoin','mc_rejoin_on','mc_rejoin_st')">💾 Save</button><span id="mc_rejoin_st" class="text-11"></span></div>
  </div>
</div>
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    <span class="mc-icon">🔗</span>
    <div class="flex-1"><span class="mc-title">Invite Tracker</span><div class="mc-desc">Track which invite link brought each member</div></div>
  ...
  <div class="mc-body">
    ...
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveInvite()">💾 Save</button><span id="mc_invite_st" class="text-11"></span></div>
  </div>
</div>
...
<div class="mc-section">
  <div class="mc-header" onclick="mcToggle(this)">
    <span class="mc-icon">🗓️</span>
    <div class="flex-1"><span class="mc-title">Scheduled Roles</span><div class="mc-desc">Give or remove roles at scheduled times</div></div>
  ...
  <div class="mc-body">
    ...
    <div class="d-flex items-center gap-4" style="margin-top:6px"><button class="mc-save" onclick="mcSaveSchedRoles()">💾 Save</button><span id="mc_schedr_st" class="text-11"></span></div>
  </div>
</div>
...
function ifSave_timezones(){
  ...
  fetch('/api/features/timezones',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_timezones_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
`;
}
...
${_inlineFeature('auto-thread', 'autoThread', 'Auto-Thread', '🧵', 'Auto-create threads on messages in configured channels.', `
  ...
  <div class="clr-muted text-10">Format: <code>channelId, nameTemplate, archiveMinutes (60-10080)</code>. Template vars: <code>{user}</code>, <code>{date}</code></div>
`, { accent: '#9c27b0' })}
<script>
...
function ifSave_auto_thread(){
  ...
  fetch('/api/features/auto-thread',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_auto_thread_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
...
${_inlineFeature('auto-purge', 'autoPurge', 'Auto-Purge', '🗑️', 'Auto-delete old messages in configured channels (also available in Auto-Mod).', `
  ...
  <div class="clr-muted text-10">Format: <code>channelId, maxAgeDays (1-14), checkIntervalHours (1-24)</code></div>
`, { accent: '#9c27b0' })}
<script>
...
<!-- ═══ Scheduling Tab ═══ -->
<div id="bm-section-scheduling" class="d-none">
...
function ifSave_scheduled_announcements(){
  ...
  fetch('/api/features/scheduled-announcements',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_scheduled_announcements_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
...
function ifSave_event_sync(){
  ...
  fetch('/api/features/event-sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_event_sync_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
</div><!-- end scheduling -->

<!-- ═══ Integrations Tab ═══ -->
<div id="bm-section-integrations" class="d-none">
...
function ifSave_webhook_forwarding(){
  ...
  fetch('/api/features/webhook-forwarding',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_webhook_forwarding_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
...
function ifSave_auto_backup_discord(){
  ...
  fetch('/api/features/auto-backup-discord',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_auto_backup_discord_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
...
${_inlineFeature('yt-community-posts', 'ytCommunityPosts', 'YouTube Community Posts', '📺', 'Track new YouTube community posts and forward them to a Discord channel.', `
  ...
  <div style="margin-top:8px;display:flex;gap:8px;align-items:center">
    ...
    <span id="ytcp_check_status" class="text-11 clr-muted"></span>
  </div>
`, { accent: '#ff0000' })}
<script>
...
function ytcpManualCheck(){
  ...
  fetch('/api/features/yt-community-posts/check',{method:'POST'}).then(function(r){return r.json()}).then(function(d){
    if(d.success){st.innerHTML='<span class="clr-green2">✅ Check complete'+(d.lastError?' ('+d.lastError+')':'')+'</span>';}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Failed')+'</span>';}
    setTimeout(function(){st.textContent=''},5000);
  }).catch(function(e){st.innerHTML='<span class="clr-red">❌ '+e.message+'</span>';});
}
</script>
</div><!-- end integrations -->

<!-- ═══ Tracking Tab ═══ -->
<div id="bm-section-tracking" class="d-none">
...
function ifSave_member_milestones(){
  ...
  fetch('/api/features/member-milestones',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(d){var st=document.getElementById('if_member_milestones_status');if(d.success){st.innerHTML='<span class="clr-green2">✅ Saved!</span>';setTimeout(function(){st.innerHTML=''},3000);}else{st.innerHTML='<span class="clr-red">❌ '+(d.error||'Error')+'</span>';}}).catch(function(e){alert(e.message)});
}
</script>
</div><!-- end tracking -->
</div><!-- end bm-wrap -->
...
<div id="feat-modal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.7);z-index:9999;align-items:center;justify-content:center">
  <div style="background:#1a1a2e;border:1px solid #3a3a42;border-radius:12px;padding:24px;max-width:600px;width:90%;max-height:80vh;overflow-y:auto">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
      <h3 id="feat-modal-title" class="m-0">Configure Feature</h3>
...
function featConfigure(api) {
  ...
  document.getElementById('feat-modal-body').innerHTML = '<span class="clr-muted">Loading...</span>';
  fetch('/api/features/' + api).then(function(r) { return r.json(); }).then(function(d) {
    if (!d.success && !d.config) { document.getElementById('feat-modal-body').innerHTML = '<span class="clr-red">Failed to load config</span>'; return; }
  ...
  }).catch(function(e) { document.getElementById('feat-modal-body').innerHTML = '<span class="clr-red">Error: ' + e.message + '</span>'; });
}
...
<div class="gi-stats-grid">
  <div class="card" style="text-align:center;padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border:1px solid #2a2f3a">
    ...
    <div class="text-12 opacity-6">Guides Indexed</div>
  </div>
  <div class="card" style="text-align:center;padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border:1px solid #2a2f3a">
    <div style="font-size:28px;font-weight:700;color:#f39c12" id="gi-stat-analyses">—</div>
    <div class="text-12 opacity-6">Analyses Run</div>
  </div>
  <div class="card" style="text-align:center;padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border:1px solid #2a2f3a">
    <div style="font-size:28px;font-weight:700;color:#e67e22" id="gi-stat-idleon">—</div>
    <div class="text-12 opacity-6">IdleOn Terms</div>
  </div>
  <div class="card" style="text-align:center;padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border:1px solid #2a2f3a">
    <div style="font-size:28px;font-weight:700;color:#1b9aaa" id="gi-stat-steam">—</div>
    <div class="text-12 opacity-6">Steam Patches</div>
  </div>
  <div class="card" style="text-align:center;padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border:1px solid #2a2f3a">
    <div style="font-size:14px;font-weight:600;color:#2ecc71" id="gi-stat-scan">Never</div>
    <div class="text-12 opacity-6">Last Scan</div>
  </div>
  <div class="card" style="text-align:center;padding:16px;background:linear-gradient(135deg,#2b2d31,#232428);border:1px solid #2a2f3a">
    <div style="font-size:14px;font-weight:600;color:#3498db" id="gi-stat-bump">Never</div>
    <div class="text-12 opacity-6">Last Bump</div>
  </div>
</div>

<!-- Config -->
<div class="card" class="mb-5" style="margin-bottom:18px">
  <h3 style="margin:0 0 12px">⚙️ Configuration</h3>
  <div style="display:flex;gap:12px;align-items:end;flex-wrap:wrap">
    <div style="flex:1;min-width:250px">
      <label class="text-12 opacity-7 d-block mb-1">Forum Channel IDs (comma-separated)</label>
      <input type="text" id="gi-channels" class="input" placeholder="123456789012345678" class="w-full">
    </div>
    <div style="width:140px">
      <label class="text-12 opacity-7 d-block mb-1">Auto-scan (hours, 0=off)</label>
      <input type="number" id="gi-autoscan" class="input" min="0" max="168" value="0" class="w-full">
    </div>
    <div style="width:140px">
      <label class="text-12 opacity-7 d-block mb-1">Auto-bump (hours)</label>
      <input type="number" id="gi-autobump-hours" class="input" min="1" max="168" value="23" class="w-full">
    </div>
  ...
  <details style="margin-top:14px">
    ...
    <div style="display:flex;gap:12px;align-items:end;flex-wrap:wrap;margin-top:10px">
      <div style="width:140px">
        <label class="text-12 opacity-7 d-block mb-1">Stagger delay (ms)</label>
        <input type="number" id="gi-bump-stagger" class="input" min="500" max="10000" value="2000" class="w-full">
      </div>
      <div style="width:140px">
        <label class="text-12 opacity-7 d-block mb-1">Skip recent (hours)</label>
        <input type="number" id="gi-bump-skip-recent" class="input" min="0" max="168" value="2" class="w-full">
      </div>
      <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer">
        <input type="checkbox" id="gi-bump-msg-on"> Send message
      </label>
      <div style="flex:1;min-width:200px">
        <label class="text-12 opacity-7 d-block mb-1">Bump message</label>
        <input type="text" id="gi-bump-msg" class="input" placeholder="📌 Keeping this guide thread active!" maxlength="200" class="w-full">
      </div>
    </div>
  </details>
  <details style="margin-top:14px">
    ...
    <div style="display:flex;gap:12px;align-items:end;flex-wrap:wrap;margin-top:10px">
      <div style="flex:1;min-width:200px">
        <label class="text-12 opacity-7 d-block mb-1">Notify Channel ID (optional override)</label>
        <input type="text" id="gi-notify-channel" class="input" placeholder="Leave empty for guide threads" class="w-full">
      </div>
      <div style="width:140px">
        <label class="text-12 opacity-7 d-block mb-1">Cooldown (hours)</label>
        <input type="number" id="gi-notify-cooldown" class="input" min="0" max="720" value="12" class="w-full">
      </div>
      <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer">
...
<div class="card" class="mb-5" style="margin-bottom:18px">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
    ...
    <span class="text-12 opacity-6" id="gi-notify-history-count"></span>
  </div>
...
<div class="card" class="mb-5" style="margin-bottom:18px">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:12px">
    <h3 class="m-0">🎮 IdleOn Game Data</h3>
    <div style="display:flex;gap:6px;align-items:center">
...
<div class="card" class="mb-5" style="margin-bottom:18px">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:12px">
    <h3 class="m-0">📖 Indexed Guides</h3>
    <div class="d-flex gap-3 items-center flex-wrap">
  ...
  <div id="gi-guides-list" class="text-13"><span style="opacity:0.6">Loading...</span></div>
</div>

<!-- Analyses History -->
<div class="card" class="mb-5" style="margin-bottom:18px">
  <h3 style="margin:0 0 12px">📊 Analysis History</h3>
  <div id="gi-analyses-list" class="text-13"><span style="opacity:0.6">Loading...</span></div>
</div>

<!-- Steam Patch Notes -->
<div class="card" class="mb-5" style="margin-bottom:18px">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:12px">
    <h3 class="m-0">🎮 Steam Patch Notes <span style="font-size:12px;opacity:0.5;font-weight:400" id="gi-steam-last-fetch"></span></h3>
    <div style="display:flex;gap:8px;align-items:center">
  ...
  <div id="gi-steam-list" class="text-13"><span style="opacity:0.6">Click "Fetch Steam Patches" to load patch notes from Steam.</span></div>
  <!-- Steam pagination -->
...
<div id="gi-patch-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9999;align-items:center;justify-content:center;padding:20px">
  <div style="background:var(--bg-card,#1e1e2e);border-radius:12px;max-width:600px;width:100%;padding:24px;position:relative">
    ...
    <label class="text-12 opacity-7 d-block mb-1">Patch Title / Version</label>
    <input type="text" id="gi-patch-title" class="input" placeholder="Patch 3.2" style="width:100%;margin-bottom:12px">
    <label class="text-12 opacity-7 d-block mb-1">Patch Notes</label>
    ...
    try {
      ...
      if (g.images && g.images.length > 0) {
        imgDiv.innerHTML = '<div style="font-size:12px;opacity:0.7;margin-bottom:6px">📷 ' + g.images.length + ' image(s) saved locally</div>' +
          ...
          g.images.map(img => '<div class="pos-rel"><img src="' + esc(img.local) + '" style="width:120px;height:80px;object-fit:cover;border-radius:6px;border:1px solid rgba(255,255,255,0.1)" title="' + esc(img.name) + '"></div>').join('') +
          '</div>';
      } else {
  ...
  function renderAnalysisDetail(a) {
    ...
    if (!a.results || a.results.length === 0) {
      ...
      for (const r of a.results) {
        ...
        html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap"><span class="text-18">' + (icons[r.confidence]||'⚪') + '</span>';
        ...
        for (const c of r.changes) {
          ...
          if (c.oldValue && c.newValue) html += ' <code class="text-11">' + esc(c.oldValue) + '</code> → <code class="text-11">' + esc(c.newValue) + '</code>';
  ...
  window.guideIndexerViewSteamPatch = async function(gid) {
    ...
    try {
      ...
      document.getElementById('gi-steam-detail-meta').innerHTML = new Date(p.date).toLocaleString() + ' — by ' + esc(p.author || '?') +
        ...
        (p.analyzedAt ? ' — <span class="clr-green2">✓ Analyzed ' + new Date(p.analyzedAt).toLocaleString() + '</span>' : '');
