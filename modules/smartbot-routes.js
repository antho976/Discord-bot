...
export function registerSmartBotRoutes(app, { smartBot, requireAuth, debouncedSaveState, saveState, checkNewsFeed }) {
  ...
  app.get('/api/smartbot/quotes', requireAuth, (req, res) => {
    res.json({ success: true, quotes: smartBot.getQuotes() });
  });
  ...
  app.post('/api/smartbot/quotes', requireAuth, (req, res) => {
    ...
    const quote = smartBot.addQuote(text, addedBy || req.session?.username || 'dashboard', tags);
    debouncedSaveState();
    res.json({ success: true, quote, total: smartBot.quotes.length });
  });

  app.put('/api/smartbot/quotes/:id', requireAuth, (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.json({ success: false, error: 'Invalid quote ID' });
    const updated = smartBot.updateQuote(id, req.body);
  ...
  app.delete('/api/smartbot/quotes/:id', requireAuth, (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return res.json({ success: false, error: 'Invalid quote ID' });
    const deleted = smartBot.deleteQuote(id);
    if (!deleted) return res.json({ success: false, error: 'Quote not found' });
    debouncedSaveState();
    res.json({ success: true, total: smartBot.quotes.length });
  });

  // Embedding stats for dashboard
  app.get('/api/smartbot/embedder/stats', requireAuth, (req, res) => {
    res.json({ success: true, stats: smartBot.embedder ? smartBot.embedder.getStats() : null });
  });
}
...
export function renderSmartBotQuotesTab(smartBot) {
  const quotesCount = smartBot.quotes?.length || 0;
  const embedStats = smartBot.embedder ? smartBot.embedder.getStats() : {};
  return `
${_sbStyles()}
${_sbToastScript()}
...
function qRender(quotes) {
  ...
  if (!quotes || quotes.length === 0) {
    el.innerHTML = '<div style="color:#8b8fa3;font-size:13px;padding:12px">No quotes yet. Add one above!</div>';
    return;
  }
  var html = '';
...
function qAdd() {
  ...
  fetch('/api/smartbot/quotes', {
    ...
    if (d.success) {
      ...
      document.getElementById('q-addedby').value = '';
...
function qDel(id) {
  ...
  fetch('/api/smartbot/quotes/' + id, { method: 'DELETE' })
    .then(function(r){return r.json()}).then(function(d) {
      if (d.success) { sbToast('Quote deleted'); qLoad(); }
      else sbToast(d.error || 'Failed');
    }).catch(function(e){alert(e.message)});
}
